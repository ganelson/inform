...
@ =
inter_error_message *Inter::Verify::defn(inter_frame P, int index) {
	inter_symbol *S = Inter::SymbolsTables::unequated_symbol_from_id(Inter::Packages::scope_of(P), P.data[index]);
	if (S == NULL) return Inter::Frame::error(&P, I"1no symbol for ID", NULL);
	if (S->equated_to) {
...
inter_error_message *Inter::Verify::local_defn(inter_frame P, int index, inter_symbols_table *T) {
	...
	if (S == NULL) return Inter::Frame::error(&P, I"2no symbol for ID", NULL);
...
inter_error_message *Inter::Verify::symbol(inter_frame P, inter_t ID, inter_t construct) {
	inter_symbol *S = Inter::SymbolsTables::symbol_from_id(Inter::Packages::scope_of(P), ID);
...
inter_error_message *Inter::Verify::symbol_KOI(inter_frame P, inter_t ID) {
	inter_symbol *S = Inter::SymbolsTables::symbol_from_id(Inter::Packages::scope_of(P), ID);
...
inter_error_message *Inter::Verify::value(inter_frame P, int index, inter_symbol *kind_symbol) {
	...
	return Inter::Types::verify(P, kind_symbol, V1, V2, Inter::Packages::scope_of(P));
}
