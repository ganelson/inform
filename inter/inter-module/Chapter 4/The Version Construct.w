...
void Inter::Version::verify(inter_construct *IC, inter_frame *P, inter_package *owner, inter_error_message **E) {
	if (P->node->W.extent != EXTENT_VERSION_IFR) { *E = Inter::Frame::error(P, I"extent wrong", NULL); return; }
	if (P->node->W.data[NUMBER_VERSION_IFLD] < 1) { *E = Inter::Frame::error(P, I"version out of range", NULL); return; }
}

void Inter::Version::write(inter_construct *IC, OUTPUT_STREAM, inter_frame *P, inter_error_message **E) {
	WRITE("version %d", P->node->W.data[NUMBER_VERSION_IFLD]);
}
