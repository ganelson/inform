...
inter_error_message *Inter::PropertyValue::new(inter_bookmark *IBM, inter_t PID, inter_t OID,
	...
	inter_tree_node *P = Inter::Frame::fill_4(IBM, PROPERTYVALUE_IST,
...
void Inter::PropertyValue::verify(inter_construct *IC, inter_tree_node *P, inter_package *owner, inter_error_message **E) {
	...
	if (P->W.extent != EXTENT_PVAL_IFR) { *E = Inter::Frame::error(P, I"extent wrong", NULL); return; }
	...
	*E = Inter::Verify::symbol_KOI(owner, P, P->W.data[OWNER_PVAL_IFLD]); if (*E) return;
	...
	if (vcount == 0) {
		...
		inter_tree_node *X;
		LOOP_THROUGH_INTER_FRAME_LIST(X, FL) {
			if (X->W.data[PROP_PVAL_IFLD] == P->W.data[PROP_PVAL_IFLD]) { *E = Inter::Frame::error(P, I"duplicate property value", NULL); return; }
			if (X->W.data[OWNER_PVAL_IFLD] != P->W.data[OWNER_PVAL_IFLD]) { *E = Inter::Frame::error(P, I"instance property list malformed", NULL); return; }
		}
