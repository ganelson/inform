...
void Inter::Primitive::read(inter_construct *IC, inter_bookmark *IBM, inter_line_parse *ilp, inter_error_location *eloc, inter_error_message **E) {
	*E = Inter::Defn::vet_level(IBM, PRIMITIVE_IST, ilp->indent_level, eloc);
	...
	inter_tree_node *F = Inter::Node::fill_1(IBM, PRIMITIVE_IST, Inter::SymbolsTables::id_from_IRS_and_symbol(IBM, prim_name), eloc, (inter_t) ilp->indent_level);
	...
	while (Regexp::match(&mr2, in, L" *(%i+) *(%c*)")) {
		...
		if (*E) return;
		if (lcat == 0) break;
		if (Inter::Node::extend(F, (inter_t) 1) == FALSE) internal_error("can't extend");
...
void Inter::Primitive::verify(inter_construct *IC, inter_tree_node *P, inter_package *owner, inter_error_message **E) {
	if (P->W.extent < MIN_EXTENT_PRIM_IFR) { *E = Inter::Node::error(P, I"primitive extent wrong", NULL); return; }
	*E = Inter::Verify::defn(owner, P, DEFN_PRIM_IFLD); if (*E) return;
	inter_symbol *prim_name = Inter::SymbolsTables::symbol_from_id(Inter::Node::globals(P), P->W.data[DEFN_PRIM_IFLD]);
	if ((prim_name == NULL) || (Str::get_first_char(prim_name->symbol_name) != '!'))
		{ *E = Inter::Node::error(P, I"primitive not beginning with '!'", NULL); return; }
	int voids = 0, args = 0;
	...
	if ((voids > 1) || ((voids == 1) && (args > 1)))
		{ *E = Inter::Node::error(P, I"if used on the left, 'void' must be the only argument", NULL); return; }
}
...
void Inter::Primitive::write(inter_construct *IC, OUTPUT_STREAM, inter_tree_node *P, inter_error_message **E) {
	...
	} else { *E = Inter::Node::error(P, I"cannot write primitive", NULL); return; }
}
