...
void Inter::Primitive::read(inter_construct *IC, inter_bookmark *IBM, inter_line_parse *ilp, inter_error_location *eloc, inter_error_message **E) {
	*E = Inter::Defn::vet_level(IBM, PRIMITIVE_IST, ilp->indent_level, eloc);
	...
	while (Regexp::match(&mr2, in, L" *(%i+) *(%c*)")) {
		...
		F->node->W.data[F->node->W.extent - 1] = lcat;
	...
	if (*E) return;
	if (Inter::Frame::extend(F, (inter_t) 1) == FALSE) internal_error("can't extend");
	F->node->W.data[F->node->W.extent - 1] = rcat;
...
void Inter::Primitive::verify(inter_construct *IC, inter_frame *P, inter_package *owner, inter_error_message **E) {
	if (P->node->W.extent < MIN_EXTENT_PRIM_IFR) { *E = Inter::Frame::error(P, I"primitive extent wrong", NULL); return; }
	*E = Inter::Verify::defn(owner, P, DEFN_PRIM_IFLD); if (*E) return;
	inter_symbol *prim_name = Inter::SymbolsTables::symbol_from_id(Inter::Frame::globals(P), P->node->W.data[DEFN_PRIM_IFLD]);
	...
	int voids = 0, args = 0;
	for (int i=CAT_PRIM_IFLD; i<P->node->W.extent-1; i++) {
		if (P->node->W.data[i] == 0) voids++;
		args++;
	}
...
void Inter::Primitive::write(inter_construct *IC, OUTPUT_STREAM, inter_frame *P, inter_error_message **E) {
	...
	if (prim_name) {
		...
		for (int i=CAT_PRIM_IFLD; i<P->node->W.extent-1; i++) {
			WRITE(" ");
			Inter::Primitive::write_category(OUT, P->node->W.data[i]);
			cats++;
		}
		if (cats == 0) WRITE(" void");
		WRITE(" -> ");
		Inter::Primitive::write_category(OUT, P->node->W.data[P->node->W.extent-1]);
...
int Inter::Primitive::arity(inter_symbol *prim) {
	...
	inter_frame *D = Inter::Symbols::definition(prim);
	if (D == NULL) return 0;
	return D->node->W.extent - CAT_PRIM_IFLD - 1;
}
...
inter_t Inter::Primitive::operand_category(inter_symbol *prim, int i) {
	...
	inter_frame *D = Inter::Symbols::definition(prim);
	if (D == NULL) return 0;
	return D->node->W.data[CAT_PRIM_IFLD + i];
}
...
inter_t Inter::Primitive::result_category(inter_symbol *prim) {
	...
	inter_frame *D = Inter::Symbols::definition(prim);
	if (D == NULL) return 0;
	return D->node->W.data[D->node->W.extent - 1];
}
