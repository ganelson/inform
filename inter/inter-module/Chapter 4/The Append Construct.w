...
void Inter::Append::verify(inter_construct *IC, inter_frame *P, inter_package *owner, inter_error_message **E) {
	...
	if (P->node->W.extent != EXTENT_APPEND_IFR) { *E = Inter::Frame::error(P, I"extent wrong", NULL); return; }
	inter_symbol *symbol = Inter::SymbolsTables::symbol_from_id(Inter::Packages::scope(owner), P->node->W.data[SYMBOL_APPEND_IFLD]);;
	if (symbol == NULL) { *E = Inter::Frame::error(P, I"no target name", NULL); return; }
	if (P->node->W.data[TEXT_APPEND_IFLD] == 0) { *E = Inter::Frame::error(P, I"no translation text", NULL); return; }

	if (vcount == 0) {
		inter_t ID = P->node->W.data[TEXT_APPEND_IFLD];
