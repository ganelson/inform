...
inter_error_message *Inter::Label::new(inter_bookmark *IBM, inter_symbol *routine, inter_symbol *lab_name, inter_t level, inter_error_location *eloc) {
	inter_tree_node *P = Inter::Frame::fill_2(IBM, LABEL_IST, 0, Inter::SymbolsTables::id_from_IRS_and_symbol(IBM, lab_name), eloc, level);
...
void Inter::Label::verify(inter_construct *IC, inter_tree_node *P, inter_package *owner, inter_error_message **E) {
	if (P->W.extent != EXTENT_LABEL_IFR) { *E = Inter::Frame::error(P, I"extent wrong", NULL); return; }
	...
	if (P->W.data[LEVEL_IFLD] < 1) { *E = Inter::Frame::error(P, I"label with bad level", NULL); return; }
