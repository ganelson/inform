...
=
void Inter::Val::read(inter_construct *IC, inter_reading_state *IRS, inter_line_parse *ilp, inter_error_location *eloc, inter_error_message **E) {
	if (ilp->no_annotations > 0) { *E = Inter::Errors::plain(I"__annotations are not allowed", eloc); return; }

	*E = Inter::Defn::vet_level(IRS, VAL_IST, ilp->indent_level, eloc);
	if (*E) return;
	...
	*E = Inter::Val::new(IRS, routine, val_kind, ilp->indent_level, val1, val2, eloc);
}
...
void Inter::Val::verify(inter_construct *IC, inter_frame P, inter_error_message **E) {
	...
	*E = Inter::Verify::symbol(P, P.data[KIND_VAL_IFLD], KIND_IST); if (*E) return;
