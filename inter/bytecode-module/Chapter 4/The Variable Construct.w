...
void VariableInstruction::define_construct(void) {
	...
	InterInstruction::defines_symbol_in_fields(IC, DEFN_VAR_IFLD, KIND_VAR_IFLD);
	InterInstruction::specify_syntax(IC, I"variable TOKENS = TOKENS");
	InterInstruction::fix_instruction_length_between(IC, EXTENT_VAR_IFR, EXTENT_VAR_IFR);
...
=
void VariableInstruction::read(inter_construct *IC, inter_bookmark *IBM, inter_line_parse *ilp, inter_error_location *eloc, inter_error_message **E) {
	...
	*E = VariableInstruction::new(IBM, InterSymbolsTable::id_from_symbol_at_bookmark(IBM, var_name), var_type, val, (inter_ti) ilp->indent_level, eloc);
}

inter_error_message *VariableInstruction::new(inter_bookmark *IBM, inter_ti VID, inter_type var_type, inter_pair val, inter_ti level, inter_error_location *eloc) {
...
void VariableInstruction::verify(inter_construct *IC, inter_tree_node *P, inter_package *owner, inter_error_message **E) {
	*E = VerifyingInter::TID_field(owner, P, KIND_VAR_IFLD); if (*E) return;
	inter_type type = InterTypes::from_TID_in_field(P, KIND_VAR_IFLD);
	*E = VerifyingInter::data_pair_fields(owner, P, VAL1_VAR_IFLD, type); if (*E) return;
}

void VariableInstruction::write(inter_construct *IC, OUTPUT_STREAM, inter_tree_node *P, inter_error_message **E) {
