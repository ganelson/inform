...
@<Create the main EPS map super-level@> =
	...
	main_eml->width = EPSMap::get_int_mp(I"minimum-map-width", NULL);
	main_eml->actual_height = 0;
	main_eml->titling_point_size = EPSMap::get_int_mp(I"title-size", NULL);
	...
	EPSMap::put_text_mp(I"title", &(main_eml->map_parameters), main_eml->titling);
...
@<Create an EPS map level for this z-slice@> =
	...
	EPSMap::put_text_mp(I"subtitle", &(eml->map_parameters), eml->titling);
...
@<Inherit EPS room colours from those used in the World Index@> =
	faux_instance *R;
	LOOP_OVER_ROOMS(R)
		EPSMap::put_text_mp(I"room-colour", IXInstances::get_parameters(R),
			R->fimd.colour);
...
void RenderEPSMap::EPS_compile_map(OUTPUT_STREAM) {
	int blh, /* total height of the EPS map area (not counting border) */
		...
		border = EPSMap::get_int_mp(I"border-size", NULL),
		vskip = EPSMap::get_int_mp(I"vertical-spacing", NULL);
	...
	RenderEPSMap::EPS_compile_header(OUT, bounding_box_width, bounding_box_height,
		EPSMap::get_text_mp(I"title-font", NULL), EPSMap::get_int_mp(I"title-size", NULL));

	if (EPSMap::get_int_mp(I"map-outline", NULL))
	...
	LOOP_OVER(eml, EPS_map_level) {
		...
		int mapunit = EPSMap::get_int_mp(I"grid-size", level_scope);
...
@<Compute the dimensions of the EPS map@> =
	...
	LOOP_BACKWARDS_OVER(eml, EPS_map_level) {
		map_parameter_scope *level_scope = &(eml->map_parameters);
		int mapunit = EPSMap::get_int_mp(I"grid-size", level_scope);
		int p = EPSMap::get_int_mp(I"title-size", level_scope);
		if (eml->contains_rooms) p = EPSMap::get_int_mp(I"subtitle-size", level_scope);
...
@<Draw the title for this EPS map level@> =
	...
	if (eml->contains_rooms) {
		if (EPSMap::get_int_mp(I"monochrome", level_scope)) RenderEPSMap::EPS_compile_set_greyscale(OUT, 0);
		else RenderEPSMap::EPS_compile_set_colour(OUT, EPSMap::get_text_mp(I"subtitle-colour", level_scope));
		RenderEPSMap::plot_stream_at(OUT,
			EPSMap::get_text_mp(I"subtitle", level_scope),
			NULL, 128,
			EPSMap::get_text_mp(I"subtitle-font", level_scope),
			border*2, y+vskip,
			EPSMap::get_int_mp(I"subtitle-size", level_scope),
			FALSE, FALSE);
	} else {
		if (EPSMap::get_int_mp(I"monochrome", level_scope)) RenderEPSMap::EPS_compile_set_greyscale(OUT, 0);
		else RenderEPSMap::EPS_compile_set_colour(OUT, EPSMap::get_text_mp(I"title-colour", level_scope));
		RenderEPSMap::plot_stream_at(OUT,
			EPSMap::get_text_mp(I"title", NULL),
			NULL, 128,
			EPSMap::get_text_mp(I"title-font", level_scope),
			border*2, y+2*vskip,
			EPSMap::get_int_mp(I"title-size", level_scope),
			FALSE, TRUE);
	}
...
@<Establish EPS coordinates for this room@> =
	...
	int offs = EPSMap::get_int_mp(I"room-offset", room_scope);
...
@<Draw the map connections from this room as EPS paths@> =
	map_parameter_scope *room_scope = IXInstances::get_parameters(R);
	RenderEPSMap::EPS_compile_line_width_setting(OUT, EPSMap::get_int_mp(I"route-thickness", room_scope));

	int bx = R->fimd.eps_x;
	int by = R->fimd.eps_y;
	int boxsize = EPSMap::get_int_mp(I"room-size", room_scope)/2;
	int R_stiffness = EPSMap::get_int_mp(I"route-stiffness", room_scope);
	int dir;
...
@<Draw a single map connection as an EPS arrow@> =
	int T_stiffness = EPSMap::get_int_mp(I"route-stiffness", IXInstances::get_parameters(T));
	if (EPSMap::get_int_mp(I"monochrome", level_scope)) RenderEPSMap::EPS_compile_set_greyscale(OUT, 0);
	else RenderEPSMap::EPS_compile_set_colour(OUT, EPSMap::get_text_mp(I"route-colour", level_scope));
...
@<Draw a one-way arrow for a distant or off-level connection@> =
	...
	RenderEPSMap::plot_text_at(OUT, NULL, T,
		EPSMap::get_int_mp(I"annotation-length", NULL),
		EPSMap::get_text_mp(I"annotation-font", NULL),
		bx+E.x*boxsize*6/scaled/5, by+E.y*boxsize*6/scaled/5,
		EPSMap::get_int_mp(I"annotation-size", NULL),
		TRUE, TRUE);
...
@<Draw the filled box for the room@> =
	...
	if (EPSMap::get_int_mp(I"monochrome", room_scope)) RenderEPSMap::EPS_compile_set_greyscale(OUT, 75);
	else RenderEPSMap::EPS_compile_set_colour(OUT, EPSMap::get_text_mp(I"room-colour", room_scope));
	RenderEPSMap::EPS_compile_room_boundary_path(OUT, bx, by, boxsize, EPSMap::get_text_mp(I"room-shape", room_scope));
	WRITE("fill\n\n");

@<Draw the outline of the box for the room@> =
	if (EPSMap::get_int_mp(I"room-outline", room_scope)) {
		RenderEPSMap::EPS_compile_line_width_setting(OUT, EPSMap::get_int_mp(I"room-outline-thickness", room_scope));
		WRITE("newpath %% Room outline\n");
		if (EPSMap::get_int_mp(I"monochrome", level_scope)) RenderEPSMap::EPS_compile_set_greyscale(OUT, 0);
		else RenderEPSMap::EPS_compile_set_colour(OUT, EPSMap::get_text_mp(I"room-outline-colour", room_scope));
		RenderEPSMap::EPS_compile_room_boundary_path(OUT, bx, by, boxsize, EPSMap::get_text_mp(I"room-shape", room_scope));
		WRITE("stroke\n");
		RenderEPSMap::EPS_compile_line_width_unsetting(OUT);
	}
...
@<Write in the name of the room@> =
	int offs = EPSMap::get_int_mp(I"room-name-offset", room_scope);
	int xpart = offs%10000, ypart = offs/10000;
	while (xpart > 5000) xpart-=10000;
	while (xpart < -5000) xpart+=10000;
	bx += xpart*mapunit/100;
	...
	if (EPSMap::get_int_mp(I"monochrome", level_scope)) RenderEPSMap::EPS_compile_set_greyscale(OUT, 0);
	else RenderEPSMap::EPS_compile_set_colour(OUT, EPSMap::get_text_mp(I"room-name-colour", room_scope));
	text_stream *legend = EPSMap::get_text_mp(I"room-name", room_scope);
	...
	RenderEPSMap::plot_text_at(OUT, legend, room_to_name,
		EPSMap::get_int_mp(I"room-name-length", room_scope),
		EPSMap::get_text_mp(I"room-name-font", room_scope),
		bx, by, EPSMap::get_int_mp(I"room-name-size", room_scope),
		TRUE, TRUE);
...
@<Plot all of the rubrics onto the EPS map@> =
	...
	LOOP_OVER(rh, rubric_holder) {
		...
		int mapunit = EPSMap::get_int_mp(I"grid-size", NULL);
		while (xpart > 5000) xpart-=10000;
		while (xpart < -5000) xpart+=10000;
		if (EPSMap::get_int_mp(I"monochrome", NULL)) RenderEPSMap::EPS_compile_set_greyscale(OUT, 0);
