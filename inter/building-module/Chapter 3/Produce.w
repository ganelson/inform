...
inter_package *Produce::block(packaging_state *save, inter_name *iname) {
	...
	if (save) {
		*save = Packaging::enter_home_of(iname);
		...
		if ((R == NULL) || (R == Packaging::main(Produce::tree()))) {
	...
	if (Packaging::housed_in_function(Produce::tree(), iname))
	...
	current_inter_routine = Produce::package(block_iname, PackageTypes::get(Produce::tree(), I"_code"));
...
inter_name *Produce::kernel(inter_name *public_name) {
	if (Packaging::housed_in_function(Produce::tree(), public_name) == FALSE)
...
inter_name *Produce::find_by_name(text_stream *name) {
	...
	inter_name *try = HierarchyLocations::find_by_name(Produce::tree(), name);
	if (try == NULL) {
		HierarchyLocations::con(-1, name, Translation::same(), HierarchyLocations::plug());
		try = HierarchyLocations::find_by_name(Produce::tree(), name);
	}
	return try;
}
