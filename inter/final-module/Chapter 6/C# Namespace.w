[CSNamespace::] C# Namespace.

How identifiers are used in the C# code we generate.

@ =
void CSNamespace::initialise(code_generator *gtr) {
	METHOD_ADD(gtr, MANGLE_IDENTIFIER_MTID, CSNamespace::mangle);
	METHOD_ADD(gtr, DECLARE_CONSTANT_MTID, CSNamespace::declare_constant);
}
...
=
void CSNamespace::mangle(code_generator *gtr, OUTPUT_STREAM, text_stream *identifier) {
...
void CSNamespace::mangle_with(code_generator *gtr, OUTPUT_STREAM, text_stream *identifier,
...
=
void CSNamespace::mangle_opcode(OUTPUT_STREAM, text_stream *opcode) {
	WRITE("i7_opcode_");
...
=
void CSNamespace::mangle_variable(OUTPUT_STREAM, text_stream *var) {
	WRITE("i7_var_%S", var);
}

@ TODO Local variable names have to be handled slightly differently. This is because
...
=
void CSNamespace::fix_locals(code_generation *gen) {
	InterTree::traverse(gen->from, CSNamespace::sweep_for_locals, gen, NULL, LOCAL_IST);
}

void CSNamespace::sweep_for_locals(inter_tree *I, inter_tree_node *P, void *state) {
...
=
void CSNamespace::declare_constant(code_generator *gtr, code_generation *gen,
	inter_symbol *const_s, int form, text_stream *val) {
	text_stream *name = InterSymbol::trans(const_s);
	int seg = cs_constants_I7CGS;
	if (Str::eq(name, I"Serial")) seg = cs_ids_and_maxima_I7CGS;
	if (Str::eq(name, I"Release")) seg = cs_ids_and_maxima_I7CGS;
	if (Str::eq(name, I"BASICINFORMKIT")) seg = cs_ids_and_maxima_I7CGS;
	if (Str::eq(name, I"DICT_WORD_SIZE")) seg = cs_ids_and_maxima_I7CGS;
	segmentation_pos saved = CodeGen::select_layered(gen, seg,
		ConstantInstruction::constant_depth(const_s));
	text_stream *OUT = CodeGen::current(gen);
	WRITE("const int ");
	CSNamespace::mangle(gtr, OUT, name);
	/* WRITE(";\n");
