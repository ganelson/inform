...
=
int CSReferences::invoke_primitive(code_generation *gen, inter_ti bip, inter_tree_node *P) {
	text_stream *OUT = CodeGen::current(gen);
	text_stream *store_form = NULL;
	switch (bip) {
		case STORE_BIP:			store_form = I"Inform.Process.i7_lvalue_SET"; break;
		case PREINCREMENT_BIP:	store_form = I"Inform.Process.i7_lvalue_PREINC"; break;
		case POSTINCREMENT_BIP:	store_form = I"Inform.Process.i7_lvalue_POSTINC"; break;
		case PREDECREMENT_BIP:	store_form = I"Inform.Process.i7_lvalue_PREDEC"; break;
		case POSTDECREMENT_BIP:	store_form = I"Inform.Process.i7_lvalue_POSTDEC"; break;
		case SETBIT_BIP:		store_form = I"Inform.Process.i7_lvalue_SETBIT"; break;
		case CLEARBIT_BIP:		store_form = I"Inform.Process.i7_lvalue_CLEARBIT"; break;
...
@<This is a reference to a word lookup@> =
	WRITE("proc.i7_change_word(");
		Vanilla::node(gen, InterTree::first_child(storage_ref)); WRITE(", ");
		Vanilla::node(gen, InterTree::second_child(storage_ref)); WRITE(", ");
	if (val_supplied) { WRITE("unchecked((short)"); VNODE_2C; WRITE(")"); } else { WRITE("0"); }
	WRITE(", %S", store_form);
	WRITE(")");

@<This is a reference to a byte lookup@> =
	WRITE("proc.i7_change_byte(");
		Vanilla::node(gen, InterTree::first_child(storage_ref)); WRITE(" + ");
		Vanilla::node(gen, InterTree::second_child(storage_ref)); WRITE(", ");
	if (val_supplied) { WRITE("(byte)"); VNODE_2C; } else { WRITE("0"); }
	WRITE(", %S", store_form);
	WRITE(")");
...
@<This is a reference to something else@> =
	switch (bip) {
		...
		case STORE_BIP:			VNODE_1C; WRITE(" = "); VNODE_2C; break;
