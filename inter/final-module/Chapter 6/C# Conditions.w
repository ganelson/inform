...
=
int CSConditions::invoke_primitive(code_generation *gen, inter_ti bip, inter_tree_node *P) {
	text_stream *OUT = CodeGen::current(gen);
	switch (bip) {
		case NOT_BIP:
			WRITE("System.Convert.ToInt32(!System.Convert.ToBoolean("); VNODE_1C; WRITE("))"); break;
		case AND_BIP:
			WRITE("System.Convert.ToInt32(System.Convert.ToBoolean("); VNODE_1C; WRITE(") && System.Convert.ToBoolean("); VNODE_2C; WRITE("))"); break;
		case OR_BIP:
			WRITE("System.Convert.ToInt32(System.Convert.ToBoolean("); VNODE_1C; WRITE(") || System.Convert.ToBoolean("); VNODE_2C; WRITE("))"); break;
		case PROPERTYEXISTS_BIP:
			CS_GEN_DATA(objdata.value_ranges_needed) = TRUE;
			CS_GEN_DATA(objdata.value_property_holders_needed) = TRUE;
		...
		case OFCLASS_BIP: case IN_BIP: case NOTIN_BIP:
			WRITE("/*lotsabips*/");
			CSConditions::comparison_r(gen, bip, NULL,
				InterTree::first_child(P), InterTree::second_child(P), 0);
			break;
		case PROPERTYVALUE_BIP:
			CSConditions::comparison_r(gen, bip, InterTree::first_child(P),
...
=
void CSConditions::comparison_r(code_generation *gen,
	...
	if (Inode::is(Y, INV_IST)) {
		if (InvInstruction::method(Y) == PRIMITIVE_INVMETH) {
			...
			if (ybip == ALTERNATIVE_BIP) {
				...
				WRITE("System.Convert.ToBoolean(");
				CSConditions::comparison_r(gen, bip, K, NULL, InterTree::first_child(Y), depth+1);
				if ((bip == NE_BIP) || (bip == NOTIN_BIP)) WRITE(") && System.Convert.ToBoolean(");
				else WRITE(") /*cr*/|| System.Convert.ToBoolean(");
				CSConditions::comparison_r(gen, bip, K, NULL, InterTree::second_child(Y), depth+1);
				WRITE(")");
				if (depth == 0) { WRITE("))"); }
				return;
			}
