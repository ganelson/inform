...
void Kinds::Dimensions::kind_rescale_division_emit_factor(kind *kindx, kind *kindy) {
	...
	int k_Y = Kinds::Behaviour::scale_factor(kindy);
	int k_O = Kinds::Behaviour::scale_factor(kindo);
	if (k_O*k_Y > k_X) { Produce::val(Emit::tree(), K_number, LITERAL_IVAL, (inter_ti) (k_O*k_Y/k_X)); Produce::up(Emit::tree()); }
	if (k_O*k_Y < k_X) { Produce::val(Emit::tree(), K_number, LITERAL_IVAL, (inter_ti) (k_X/k_O/k_Y)); Produce::up(Emit::tree()); }
}
#endif
