...
void Chronology::compile_synoptic_resources(void) {
	if (PluginManager::active(chronology_plugin)) {
		@<Provide placeholder for the TIMEDEVENTSTABLE constant@>;
		@<Provide placeholder for the TIMEDEVENTTIMESTABLE array@>;
		@<Provide placeholder for the PASTACTIONSI6ROUTINES array@>;
		@<Provide placeholder for the NO_PAST_TENSE_CONDS constant@>;
		@<Provide placeholder for the NO_PAST_TENSE_ACTIONS constant@>;
		@<Provide placeholder for the TESTSINGLEPASTSTATE function@>;
	}
}

@<Provide placeholder for the TIMEDEVENTSTABLE constant@> =
	inter_name *iname = Hierarchy::find(TIMEDEVENTSTABLE_HL);
	Produce::annotate_i(iname, SYNOPTIC_IANN, TIMEDEVENTSTABLE_SYNID);
...
@<Provide placeholder for the TIMEDEVENTTIMESTABLE array@> =
	...
	Produce::annotate_i(iname, SYNOPTIC_IANN, TIMEDEVENTTIMESTABLE_SYNID);
	packaging_state save = EmitArrays::begin_table(iname, K_value);
	EmitArrays::end(save);
	Hierarchy::make_available(iname);
...
@<Provide placeholder for the PASTACTIONSI6ROUTINES array@> =
	...
	Produce::annotate_i(iname, SYNOPTIC_IANN, PASTACTIONSI6ROUTINES_SYNID);
...
@<Provide placeholder for the NO_PAST_TENSE_ACTIONS constant@> =
	...
	Produce::annotate_i(iname, SYNOPTIC_IANN, NO_PAST_TENSE_ACTIONS_SYNID);
	Hierarchy::make_available(iname);
	Emit::numeric_constant(iname, 0);

@<Provide placeholder for the TESTSINGLEPASTSTATE function@> =
	inter_name *iname = Hierarchy::find(TESTSINGLEPASTSTATE_HL);
	Produce::annotate_i(iname, SYNOPTIC_IANN, TESTSINGLEPASTSTATE_SYNID);
	packaging_state save = Functions::begin(iname);
	inter_symbol *past_flag_s = LocalVariables::new_other_as_symbol(I"past_flag");
	LocalVariables::new_other_as_symbol(I"pt");
	LocalVariables::new_other_as_symbol(I"turn_end");
	LocalVariables::new_other_as_symbol(I"wanted");
	LocalVariables::new_internal_as_symbol(I"old");
	LocalVariables::new_internal_as_symbol(I"new");
	LocalVariables::new_internal_as_symbol(I"trips");
	LocalVariables::new_internal_as_symbol(I"consecutives");
	inter_symbol *prcr_s = InterSymbolsTables::create_with_unique_name(past_flag_s->owning_table, I"prcr");
	inter_name *prcr_iname = Hierarchy::find(PRESENT_CHRONOLOGICAL_RECORD_HL);
	InterSymbolsTables::equate(prcr_s, InterNames::to_symbol(prcr_iname));
	inter_symbol *pacr_s = InterSymbolsTables::create_with_unique_name(past_flag_s->owning_table, I"pacr");
	inter_name *pacr_iname = Hierarchy::find(PAST_CHRONOLOGICAL_RECORD_HL);
	InterSymbolsTables::equate(pacr_s, InterNames::to_symbol(pacr_iname));
