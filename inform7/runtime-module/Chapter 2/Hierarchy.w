...
associated array.) The following creates a package in the "enclosure"
surrounding the current emission position -- an "enclosure" is a package of
sufficient grandness that it is expected to contain all of its own needs.

=
inter_name *Hierarchy::iname_in_enclosure(int hap, int hl) {
	package_request *PR = HierarchyLocations::attach_new_package(
		Emit::tree(), NULL, Packaging::enclosure(Emit::tree()), hap);
...
=
inter_name *Hierarchy::new_block_constant_iname(void) {
	return Hierarchy::iname_in_enclosure(BLOCK_CONSTANTS_HAP, BLOCK_CONSTANT_HL);
}
