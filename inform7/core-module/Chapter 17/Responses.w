...
response_message *Strings::response_cue(value_holster *VH, rule *owner, int marker,
	...
	if (VH) {
		if (Holsters::data_acceptable(VH)) {
			Produce::val_iname(K_value, Strings::response_launcher_iname(resp));
		}
	}
	return resp;
}
...
@<If the response is via I6, compile the necessary routine for this rule@> =
	...
	Produce::down();
		...
		Produce::down();
			...
			Produce::down();
				Produce::val_symbol(K_value, code_s);
				Produce::val(K_number, LITERAL_IVAL, (inter_t) 'a');
			Produce::up();
			Produce::inv_primitive(Produce::opcode(LE_BIP));
			Produce::down();
				Produce::val_symbol(K_value, code_s);
				Produce::val(K_number, LITERAL_IVAL, (inter_t) 'z');
			Produce::up();
		Produce::up();
		Produce::code();
		Produce::down();
			...
			Produce::down();
				Produce::ref_symbol(K_value, f_s);
				Produce::val(K_truth_state, LITERAL_IVAL, 1);
			Produce::up();
			Produce::inv_primitive(Produce::opcode(STORE_BIP));
			Produce::down();
				Produce::ref_symbol(K_value, code_s);
				Produce::inv_primitive(Produce::opcode(MINUS_BIP));
				Produce::down();
					Produce::val_symbol(K_value, code_s);
					Produce::val(K_number, LITERAL_IVAL, (inter_t) ('a'-'A'));
				Produce::up();
			Produce::up();
		Produce::up();
	Produce::up();
	...
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_symbol(K_value, s_s);
		Produce::val_iname(K_object, Hierarchy::find(NOUN_HL));
	Produce::up();
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_symbol(K_value, s2_s);
		Produce::val_iname(K_value, Hierarchy::find(SECOND_HL));
	Produce::up();
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_symbol(K_value, s3_s);
		Produce::val_iname(K_object, Hierarchy::find(PARSED_NUMBER_HL));
	Produce::up();
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_iname(K_object, Hierarchy::find(NOUN_HL));
		Produce::val_symbol(K_value, val_s);
	Produce::up();
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_iname(K_object, Hierarchy::find(SECOND_HL));
		Produce::val_symbol(K_value, val2_s);
	Produce::up();
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		...
		Produce::down();
			...
			LOOP_OVER(r2, response_message) {
				if (r2->responding_rule == resp->responding_rule) {
					...
					Produce::down();
						Produce::val(K_number, LITERAL_IVAL, (inter_t) ('A' + r2->response_marker));
						Produce::code();
						Produce::down();
							Produce::inv_primitive(Produce::opcode(STORE_BIP));
							Produce::down();
								Produce::ref_symbol(K_value, str_s);
								Produce::val_iname(K_value, r2->resp_iname);
							Produce::up();
						Produce::up();
					Produce::up();
	...
	Produce::down();
		Produce::inv_primitive(Produce::opcode(AND_BIP));
		Produce::down();
			Produce::val_symbol(K_value, str_s);
			Produce::inv_primitive(Produce::opcode(EQ_BIP));
			Produce::down();
				Produce::val_symbol(K_value, f_s);
				Produce::val(K_truth_state, LITERAL_IVAL, 0);
			Produce::up();
		Produce::up();
		Produce::code();
		Produce::down();
			Produce::inv_call_iname(Hierarchy::find(TEXT_TY_SAY_HL));
			Produce::down();
				Produce::val_symbol(K_value, str_s);
			Produce::up();
		Produce::up();
	...
	Produce::down();
		Produce::ref_iname(K_object, Hierarchy::find(NOUN_HL));
		Produce::val_symbol(K_value, s_s);
	Produce::up();
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_iname(K_object, Hierarchy::find(SECOND_HL));
		Produce::val_symbol(K_value, s2_s);
	Produce::up();
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_iname(K_object, Hierarchy::find(PARSED_NUMBER_HL));
		Produce::val_symbol(K_value, s3_s);
	Produce::up();
...
@<Compile the PrintResponse routine@> =
	...
	LOOP_OVER(resp, response_message) {
		...
		Produce::down();
			...
			Produce::code();
			Produce::down();
				Produce::inv_call_iname(Rules::RulePrintingRule());
				Produce::down();
					Produce::val_iname(K_value, Rules::iname(resp->responding_rule));
				Produce::up();
				Produce::inv_primitive(Produce::opcode(PRINT_BIP));
				Produce::down();
					Produce::val_text(I" response (");
				Produce::up();
				Produce::inv_primitive(Produce::opcode(PRINTCHAR_BIP));
				Produce::down();
					Produce::val(K_number, LITERAL_IVAL, (inter_t) ('A' + resp->response_marker));
				Produce::up();
				Produce::inv_primitive(Produce::opcode(PRINT_BIP));
				Produce::down();
					Produce::val_text(I")");
