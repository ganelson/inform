...
void Invocations::Inline::csi_inline_inner(value_holster *VH, inter_schema *sch, csi_state *CSIS) {
	...
	InterSchemas::emit(VH, sch, CSIS, to_code, to_val,
...
@<Issue PM_BadInlineExpansion problem@> =
	*X = PROBLEM_INSUB;
	...
	Problems::Issue::handmade_problem(_p_(PM_BadInlineExpansion));
...
@<Expand a bracing containing natural language text@> =
	...
	switch (<<r>>) {
		case OPTS_INSUB:
			Emit::val(K_number, LITERAL_IVAL, (inter_t) current_opts);
			break;
		case OPT_INSUB:
			if (current_opts & <<opt>>) Emit::val(K_number, LITERAL_IVAL, 1); else Emit::val(K_number, LITERAL_IVAL, 0);
			break;
		case LOCAL_INSUB: {
...
@<If the token has to be an lvalue, reject it if it isn't@> =
	if (require_to_be_lvalue) {
		...
		if (((nlv) && (NonlocalVariables::is_constant(nlv))) ||
			...
			Problems::Issue::handmade_problem(_p_(PM_NotAnLvalue));
			Problems::issue_problem_segment(
...
@<Issue problem for no natural choice@> =
	...
	Problems::Issue::handmade_problem(_p_(PM_NoNaturalDefault2));
	Problems::issue_problem_segment(
...
@<Inline command "next-routine"@> =
	kind *K = Invocations::Inline::parse_bracing_operand_as_kind(sche->operand, ParseTree::get_kind_variable_declarations(inv));
	if (K) Emit::val_iname(K_value, Kinds::Behaviour::get_inc_iname(K));
	else @<Issue an inline no-such-kind problem@>;
	return;

@<Inline command "previous-routine"@> =
	kind *K = Invocations::Inline::parse_bracing_operand_as_kind(sche->operand, ParseTree::get_kind_variable_declarations(inv));
	if (K) Emit::val_iname(K_value, Kinds::Behaviour::get_dec_iname(K));
	else @<Issue an inline no-such-kind problem@>;
	return;

@<Inline command "printing-routine"@> =
	kind *K = Invocations::Inline::parse_bracing_operand_as_kind(sche->operand, ParseTree::get_kind_variable_declarations(inv));
	if (K) Emit::val_iname(K_value, Kinds::Behaviour::get_iname(K));
	else @<Issue an inline no-such-kind problem@>;
	return;
...
@<Inline command "backspace"@> =
	Problems::quote_source(1, current_sentence);
	Problems::Issue::handmade_problem(_p_(PM_BackspaceWithdrawn));
	Problems::issue_problem_segment(
...
@<Inline command "counter"@> =
	Emit::val(K_number, LITERAL_IVAL, (inter_t) JumpLabels::read_counter(sche->operand, NOT_APPLICABLE));
	return;

@ We can also output just the storage array:

@<Inline command "counter-storage"@> =
	Emit::val_iname(K_value, JumpLabels::storage(sche->operand));
	return;
...
@<Inline annotation "box-quotation-text"@> =
	if (Rvalues::is_CONSTANT_of_kind(supplied, K_text) == FALSE) {
		...
		Problems::Issue::handmade_problem(_p_(PM_Misboxed));
		Problems::issue_problem_segment(
...
@<Issue a problem for returning a value when none was asked@> =
	...
	if (returning_from_rule) {
		Problems::Issue::handmade_problem(_p_(PM_RuleNotAllowedOutcome));
		Problems::issue_problem_segment(
		...
		Problems::Issue::handmade_problem(_p_(PM_RedundantReturnKOV));
		Problems::issue_problem_segment(
...
@<Issue a problem for returning a value of the wrong kind@> =
	...
	if (returning_from_rule) {
		Problems::Issue::handmade_problem(_p_(PM_RuleOutcomeWrongKind));
		Problems::issue_problem_segment(
			"You wrote %1 as the outcome of a rule which produces a value, but this "
			"was the wrong kind of value: %2 rather than %3.");
		Problems::issue_problem_end();
	} else {
		Problems::Issue::handmade_problem(_p_(PM_ReturnWrongKind));
		Problems::issue_problem_segment(
...
@<Inline annotation "mark-event-used"@> =
	if (Rvalues::is_CONSTANT_construction(supplied, CON_rule)) {
		...
		Problems::Issue::handmade_problem(_p_(PM_NonconstantEvent));
		Problems::issue_problem_segment(
...
@<Throw a problem message for an invalid inline annotation@> =
	...
	Problems::Issue::handmade_problem(_p_(PM_BadInlineTag));
	Problems::issue_problem_segment(
		"I attempted to compile %1 using its inline definition, "
...
@<Inline command "my"@> =
	...
	inter_symbol *lvar_s = LocalVariables::declare_this(lvar, FALSE, 8);
	if (prim_cat == REF_PRIM_CAT) Emit::ref_symbol(K_value, lvar_s);
	else Emit::val_symbol(K_value, lvar_s);
	return;
...
@<Inline command "initialise"@> =
	...
	if (Kinds::Behaviour::uses_pointer_values(K)) {
		...
		if (rv == FALSE) {
			Problems::quote_source(1, current_sentence);
			Problems::quote_kind(2, K);
			Problems::Issue::handmade_problem(_p_(PM_NoNaturalDefault));
			Problems::issue_problem_segment(
				"To achieve %1, we'd need to be able to store a default value of "
				"the kind '%2', but there's no natural choice for this.");
			Problems::issue_problem_end();
...
@<Check that increment or decrement make sense@> =
	if (Kinds::Behaviour::is_quasinumerical(K1) == FALSE) {
		...
		Problems::Issue::handmade_problem(_p_(PM_CantIncrementKind));
		Problems::issue_problem_segment(
...
@<Inline command "segment-count"@> =
	Emit::val(K_number, LITERAL_IVAL, (inter_t) ParseTree::int_annotation(inv, ssp_segment_count_ANNOT));
	return;
...
@<Inline command "final-segment-marker"@> =
	if (ParseTree::int_annotation(inv, ssp_closing_segment_wn_ANNOT) == -1) {
		...
		inter_symbol *T_s = InterSchemas::find_identifier_text(T);
...
@<Inline command "rescale"@> =
	Problems::quote_source(1, current_sentence);
	Problems::Issue::handmade_problem(_p_(PM_RescaleWithdrawn));
	Problems::issue_problem_segment(
		"I attempted to compile %1 using its inline definition, "
...
@<Primitive "solve-equation"@> =
	if (Rvalues::is_CONSTANT_of_kind(tokens->args[1], K_equation)) {
		...
		Problems::Issue::sentence_problem(_p_(PM_SolvedNameless),
