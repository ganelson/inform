...
int Routines::ToPhrases::compare(phrase *ph1, phrase *ph2) {
	...
	if (r == CONFLICTED_PH) {
		...
		Problems::Issue::handmade_problem(_p_(PM_ConflictedReturnKinds));
...
@<Issue a problem message for undetermined kinds@> =
	...
	Problems::Issue::handmade_problem(_p_(PM_UndeterminedKind));
	if (Wordings::empty(W)) {
		Problems::issue_problem_segment(
...
inter_name *Routines::ToPhrases::make_iname(phrase *ph, kind *req_kind) {
	if (Phrases::TypeData::invoked_inline(ph)) {
		...
		if (found == FALSE) {
			...
			Problems::Issue::handmade_problem(_p_(PM_PhraseNamedI6Failed));
			Problems::issue_problem_segment(
		...
		inter_name *symb = InterNames::extern_name(EXTERNALLY_DEFINED_PHRASE_INAMEF, identifier, NULL);
