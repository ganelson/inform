...
@<Check the tree position makes sense, and tell the debugging log@> =
	if (A) Problems::Issue::s_subtree_error_set_position(A);
...
@<Find meaning of the VP as a relation and a parity@> =
	...
	if (subject_phrase_subtree == NULL) Problems::Issue::s_subtree_error("SP subtree null");
	parse_node *verb_phrase_subtree = B;
	if (verb_phrase_subtree == NULL) Problems::Issue::s_subtree_error("VP subtree null");
	if (verb_phrase_subtree->down == NULL) Problems::Issue::s_subtree_error("VP subtree broken");
