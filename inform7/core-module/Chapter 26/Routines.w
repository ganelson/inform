...
packaging_state Routines::begin_framed(inter_name *iname, ph_stack_frame *phsf) {
	...
	currently_compiling_inter_block = Produce::block(&save, iname);
...
void Routines::end(packaging_state save) {
	...
	if ((currently_compiling_in_frame->allocated_pointers) ||
		...
		kernel_name = Produce::kernel(public_name);
...
@<Compile an outer shell routine with the public-facing name@> =
	...
	inter_package *block_package = Produce::block(NULL, public_name);
...
@<Compile some setup code to make ready for the kernel@> =
	Produce::push(Hierarchy::find(I7SFRAME_HL));
	...
	Produce::down();
	Produce::val(K_number, LITERAL_IVAL, (inter_t) NBV);
	Produce::up();
	...
	for (int i=0; i<currently_compiling_in_frame->no_formal_parameters_needed; i++) {
		...
		Produce::push(NonlocalVariables::iname(nlv));
	}

@<Compile a call to the kernel@> =
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
	Produce::ref_symbol(K_value, I7RBLK_symbol);
	if (returns_block_value) {
		inter_name *iname = Hierarchy::find(BLKVALUECOPY_HL);
		Produce::inv_call_iname(iname);
		Produce::down();
		Produce::val_symbol(K_number,I7RBLK_symbol);
	}

	Produce::inv_call_iname(kernel_name);
	Produce::down();
...
@<Compile some teardown code now that the kernel has finished@> =
	for (int i=currently_compiling_in_frame->no_formal_parameters_needed-1; i>=0; i--) {
		nonlocal_variable *nlv = NonlocalVariables::temporary_formal(i);
		Produce::pull(NonlocalVariables::iname(nlv));
	}
	...
	for (pointer_allocation *pall=currently_compiling_in_frame->allocated_pointers; pall; pall=pall->next_in_frame) {
		...
		Produce::inv_call_iname(iname);
		Produce::down();
		Produce::val(K_value, LITERAL_IVAL, (inter_t) pall->offset_index);
		Produce::up();
	}

	Produce::pull(Hierarchy::find(I7SFRAME_HL));

@<Compile a return from the outer shell@> =
	Produce::inv_primitive(Produce::opcode(RETURN_BIP));
	Produce::down();
		Produce::val_symbol(K_value, I7RBLK_symbol);
	Produce::up();
