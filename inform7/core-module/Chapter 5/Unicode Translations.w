...
void UnicodeTranslations::unicode_translates(parse_node *pn) {
	...
	if ((<<r>> != -1) && (<<r>> != cc)) {
		Problems::Issue::sentence_problem(_p_(PM_UnicodeAlready),
...
@<Issue PM_UnicodeNonLiteral problem@> =
	Problems::Issue::sentence_problem(_p_(PM_UnicodeNonLiteral),
...
int UnicodeTranslations::char_in_range(int cc) {
	if ((cc < 0) || (cc >= 0x10000)) {
		Problems::Issue::sentence_problem(_p_(PM_UnicodeOutOfRange),
