...
void LiteralPatterns::gpr(gpr_kit *gprk, literal_pattern *lp) {
	...
	Produce::down();
		Produce::ref_symbol(K_value, gprk->wpos_s);
		Produce::val(K_number, LITERAL_IVAL, 0);
	Produce::up();
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_symbol(K_value, gprk->mid_word_s);
		Produce::val(K_truth_state, LITERAL_IVAL, 0);
	Produce::up();
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_symbol(K_value, gprk->matched_number_s);
		Produce::val(K_number, LITERAL_IVAL, 0);
	Produce::up();

	TEMPORARY_TEXT(SL);
	WRITE_TO(SL, ".Succeeded_LP_%d", label);
	inter_symbol *succeeded_label = Produce::reserve_label(SL);
	DISCARD_TEXT(SL);
	TEMPORARY_TEXT(FL);
	WRITE_TO(FL, ".Failed_LP_%d", label);
	inter_symbol *failed_label = Produce::reserve_label(FL);
	DISCARD_TEXT(FL);
	...
	for (tc=0, ec=0; tc<lp->no_lp_tokens; tc++) {
		...
		Produce::down();
			...
			Produce::down();
				Produce::val_symbol(K_value, gprk->mid_word_s);
				Produce::val(K_truth_state, LITERAL_IVAL, 0);
			Produce::up();
			Produce::code();
			Produce::down();
				Produce::inv_primitive(Produce::opcode(STORE_BIP));
				Produce::down();
					Produce::ref_symbol(K_value, gprk->cur_word_s);
					Produce::inv_call_iname(Hierarchy::find(NEXTWORDSTOPPED_HL));
				Produce::up();
				Produce::inv_primitive(Produce::opcode(POSTDECREMENT_BIP));
				Produce::down();
				...
				Produce::down();
					Produce::inv_primitive(Produce::opcode(EQ_BIP));
					Produce::down();
						Produce::val_symbol(K_value, gprk->cur_word_s);
	...
	Produce::place_label(succeeded_label);
	...
	Produce::down();
		...
		Produce::down();
			Produce::val_symbol(K_value, gprk->sgn_s);
			Produce::val(K_number, LITERAL_IVAL, 0);
		Produce::up();
		Produce::code();
		Produce::down();
			...
			Produce::down();
				...
				if (Kinds::FloatingPoint::uses_floating_point(lp->kind_specified)) {
					...
					Produce::down();
						Produce::val_symbol(K_value, gprk->matched_number_s);
						Produce::val(K_number, LITERAL_IVAL, 0x80000000);
					Produce::up();
				} else {
	...
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_iname(K_object, Hierarchy::find(PARSED_NUMBER_HL));
		Produce::val_symbol(K_value, gprk->matched_number_s);
	Produce::up();
	...
	Produce::down();
		...
		Produce::down();
			...
			Produce::down();
				...
				Produce::down();
					Produce::val_iname(K_value, Hierarchy::find(PARSER_TRACE_HL));
				...
				Produce::down();
					...
					Produce::down();
						Produce::val_text(I"  [parsed value ");
					Produce::up();
					Produce::inv_primitive(Produce::opcode(PRINTNUMBER_BIP));
					Produce::down();
						Produce::val_iname(K_object, Hierarchy::find(PARSED_NUMBER_HL));
					Produce::up();
					Produce::inv_primitive(Produce::opcode(PRINT_BIP));
					Produce::down();
						TEMPORARY_TEXT(EXP);
						WRITE_TO(EXP, " by: %W]\n", lp->prototype_text);
						Produce::val_text(EXP);
						DISCARD_TEXT(EXP);
					Produce::up();
				Produce::up();
			Produce::up();
	...
	Produce::up();
	Produce::place_label(failed_label);
}
...
@<Compile I6 code to match a character token within a literal pattern@> =
	...
	Produce::inv_primitive(Produce::opcode(IF_BIP));
	Produce::down();
		if (upper_form != lower_form) { Produce::inv_primitive(Produce::opcode(AND_BIP)); Produce::down(); }
		Produce::inv_primitive(Produce::opcode(NE_BIP));
		Produce::down();
			Produce::inv_primitive(Produce::opcode(LOOKUPBYTE_BIP));
			Produce::down();
				Produce::val_symbol(K_value, gprk->cur_addr_s);
				Produce::inv_primitive(Produce::opcode(POSTINCREMENT_BIP));
				Produce::down();
					Produce::ref_symbol(K_value, gprk->wpos_s);
				Produce::up();
			Produce::up();
			Produce::val(K_number, LITERAL_IVAL, (inter_t) lower_form);
		Produce::up();
		if (upper_form != lower_form) {
			...
			Produce::down();
				...
				Produce::down();
					...
					Produce::down();
						Produce::val_symbol(K_value, gprk->wpos_s);
						Produce::val(K_number, LITERAL_IVAL, 1);
					Produce::up();
				Produce::up();
				Produce::val(K_number, LITERAL_IVAL, (inter_t) upper_form);
			Produce::up();
		}
		if (upper_form != lower_form) { Produce::up(); }
		Produce::code();
		Produce::down();
			Produce::inv_primitive(Produce::opcode(JUMP_BIP));
			Produce::down();
...
@<Compile I6 code to match an element token within a literal pattern@> =
	...
	if (ec == 1) {
		Produce::inv_primitive(Produce::opcode(STORE_BIP));
		Produce::down();
			Produce::ref_symbol(K_value, gprk->sgn_s);
			Produce::val(K_number, LITERAL_IVAL, 1);
		Produce::up();
	}
	Produce::inv_primitive(Produce::opcode(IF_BIP));
	Produce::down();
		...
		Produce::down();
			...
			Produce::down();
				...
				Produce::val_symbol(K_value, gprk->wpos_s);
			Produce::up();
			Produce::val(K_number, LITERAL_IVAL, (inter_t) '-');
		Produce::up();
		Produce::code();
		Produce::down();
...
@<Compile I6 code to match a real number here@> =
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_symbol(K_value, gprk->f_s);
		Produce::val(K_truth_state, LITERAL_IVAL, 0);
	Produce::up();
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_symbol(K_value, gprk->x_s);
	...
	Produce::down();
		Produce::inv_primitive(Produce::opcode(EQ_BIP));
		Produce::down();
			Produce::val_symbol(K_value, gprk->f_s);
			Produce::val(K_number, LITERAL_IVAL, 0);
		Produce::up();
		Produce::code();
		Produce::down();
	...
	Produce::down();
		...
		Produce::down();
			Produce::val_symbol(K_value, gprk->wpos_s);
			Produce::val_symbol(K_value, gprk->cur_len_s);
		Produce::up();
		Produce::code();
		Produce::down();
			Produce::inv_primitive(Produce::opcode(POSTINCREMENT_BIP));
			...
			Produce::down();
				...
				Produce::down();
					Produce::val_symbol(K_value, gprk->cur_word_s);
					Produce::val_iname(K_value, Hierarchy::find(THEN1__WD_HL));
				Produce::up();
				Produce::code();
				Produce::down();
					Produce::inv_primitive(Produce::opcode(STORE_BIP));
	...
	Produce::inv_primitive(Produce::opcode(STORE_BIP));
	Produce::down();
		Produce::ref_symbol(K_value, gprk->tot_s);
		Produce::val(K_number, LITERAL_IVAL, 0);
	Produce::up();
		...
		Produce::down();
			...
			Produce::down();
				...
				Produce::down();
					...
					Produce::val_symbol(K_value, gprk->wpos_s);
				Produce::up();
				Produce::val(K_number, LITERAL_IVAL, (inter_t) 'x');
			Produce::up();
		Produce::up();
		Produce::code();
		Produce::down();
			...
			Produce::down();
				...
				Produce::down();
					...
					Produce::down();
						Produce::val_symbol(K_value, gprk->f_s);
						Produce::val_symbol(K_value, gprk->tot_s);
					Produce::up();
					Produce::val(K_number, LITERAL_IVAL, 1);
				Produce::up();
			Produce::up();
						...
						Produce::down();
							...
							Produce::down();
								...
								Produce::down();
									...
									Produce::up();
								Produce::up();
								Produce::val(K_number, LITERAL_IVAL, (inter_t) '0');
							Produce::up();
							Produce::inv_primitive(Produce::opcode(EQ_BIP));
							Produce::down();
								Produce::inv_primitive(Produce::opcode(LOOKUPBYTE_BIP));
	...
	Produce::down();
		Produce::ref_symbol(K_value, gprk->x_s);
		Produce::inv_call_iname(Hierarchy::find(FLOATPARSE_HL));
		Produce::down();
...
@<March forwards through decimal digits@> =
	...
	Produce::down();
		...
		Produce::down();
			...
			Produce::inv_primitive(Produce::opcode(GE_BIP));
			Produce::down();
				Produce::inv_call_iname(Hierarchy::find(DIGITTOVALUE_HL));
				Produce::down();
					Produce::inv_primitive(Produce::opcode(LOOKUPBYTE_BIP));
					Produce::down();
						Produce::val_symbol(K_value, gprk->cur_addr_s);
...
@<Compile I6 code to match an integer here@> =
	...
	if (lpe->element_index > 0) {
		...
		Produce::down();
			...
			Produce::down();
				Produce::val_symbol(K_value, gprk->tot_s);
				Produce::val(K_number, LITERAL_IVAL, (inter_t) lpe->element_range);
			Produce::up();
			Produce::code();
			Produce::down();
				Produce::inv_primitive(Produce::opcode(JUMP_BIP));
			...
			Produce::down();
				...
				Produce::down();
					...
					Produce::down();
						...
						Produce::down();
							Produce::ref_symbol(K_value, gprk->f_s);
							Produce::val(K_number, LITERAL_IVAL, (inter_t) M);
						Produce::up();
						Produce::inv_primitive(Produce::opcode(WHILE_BIP));
						Produce::down();
							Produce::inv_primitive(Produce::opcode(AND_BIP));
...
@<Compile I6 code to enter mid-word parsing if not already in it@> =
	...
	Produce::down();
		...
		Produce::down();
			...
			Produce::inv_primitive(Produce::opcode(STORE_BIP));
			Produce::down();
				Produce::ref_symbol(K_value, gprk->cur_addr_s);
				Produce::inv_call_iname(Hierarchy::find(WORDADDRESS_HL));
				Produce::down();
					Produce::val_iname(K_value, Hierarchy::find(WN_HL));
				Produce::up();
			Produce::up();
			Produce::inv_primitive(Produce::opcode(STORE_BIP));
			Produce::down();
				Produce::ref_symbol(K_value, gprk->cur_len_s);
				Produce::inv_call_iname(Hierarchy::find(WORDLENGTH_HL));
				Produce::down();
					Produce::val_iname(K_value, Hierarchy::find(WN_HL));
				Produce::up();
			Produce::up();
		...
		if (Wordings::nonempty(lpn->notation_name)) {
			...
			if (k > 0) {
				...
				Produce::down();
					Produce::inv_primitive(Produce::opcode(EQ_BIP));
					Produce::down();
						Produce::val_symbol(K_value, which_s);
						Produce::val(K_number, LITERAL_IVAL, (inter_t) (lpn->allocation_id + 1));
					Produce::up();
					Produce::code();
					Produce::down();
...
@<Compile code to jump to the correct printing pattern@> =
	...
	if ((lpb) && (lpb->singular_form_only == FALSE)) {
		...
		Produce::down();
			...
			Produce::down();
				Produce::inv_primitive(Produce::opcode(JUMP_BIP));
				Produce::down();
					Produce::lab(LiteralPatterns::jump_label(lpb));
				Produce::up();
			Produce::up();
		Produce::up();
	}
	...
	for (lp = lp_list; lp; lp = lp->next_for_this_kind) {
		if (lp->marked_for_printing) {
			inter_symbol *op = Produce::opcode(GE_BIP); last_lp = lp;
			if (lp->primary_alternative) { last_primary = lp; }
			if (lp->singular_form_only) { last_singular = lp; op = Produce::opcode(EQ_BIP); }
			if (lp->plural_form_only) { last_plural = lp; op = Produce::opcode(GT_BIP); }
			Produce::inv_primitive(Produce::opcode(IF_BIP));
			Produce::down();
				...
				Produce::down();
					Produce::inv_primitive(Produce::opcode(JUMP_BIP));
					Produce::down();
						Produce::lab(LiteralPatterns::jump_label(lp));
					Produce::up();
				Produce::up();
			Produce::up();
		}
	...
	if (last_lp) {
		if ((last_lp->singular_form_only) &&
			...
			Produce::inv_primitive(Produce::opcode(JUMP_BIP));
			Produce::down();
				Produce::lab(LiteralPatterns::jump_label(last_plural));
			Produce::up();
		}
		Produce::inv_primitive(Produce::opcode(JUMP_BIP));
		Produce::down();
			Produce::lab(LiteralPatterns::jump_label(last_lp));
		Produce::up();
	}
...
@<Print according to this particular literal pattern@> =
	...
	LiteralPatterns::comment_use_of_lp(lp);
	Produce::place_label(LiteralPatterns::jump_label(lp));
				...
				Produce::down();
					...
					Produce::down();
						...
						Produce::down();
							...
							Produce::down();
								Produce::val_symbol(K_value, value_s);
								Produce::val(K_number, LITERAL_IVAL, (inter_t) (lpe->element_multiplier));
							Produce::up();
							Produce::val(K_number, LITERAL_IVAL, 0);
						Produce::up();
					Produce::up();
			...
			if (lp->number_signed) {
				...
				Produce::down();
					...
					Produce::down();
						Produce::inv_primitive(Produce::opcode(STORE_BIP));
						Produce::down();
							Produce::ref_symbol(K_value, value_s);
							Produce::inv_primitive(Produce::opcode(MINUS_BIP));
							Produce::down();
								Produce::val(K_number, LITERAL_IVAL, 0);
								Produce::val_symbol(K_value, value_s);
						...
						Produce::down();
							...
							Produce::down();
								...
								Produce::down();
									...
									Produce::val(K_number, LITERAL_IVAL, (inter_t) (lpe->element_range));
								Produce::up();
								Produce::val(K_number, LITERAL_IVAL, (inter_t) (pow));
							Produce::up();
							Produce::code();
							Produce::down();
								Produce::inv_primitive(Produce::opcode(PRINT_BIP));
								Produce::down();
									Produce::val_text(I"0");
								Produce::up();
							Produce::up();
						Produce::up();
		...
		if (ec == 0) {
			...
			Produce::down();
				...
				Produce::down();
					...
					Produce::down();
						Produce::val_symbol(K_value, value_s);
						Produce::val(K_number, LITERAL_IVAL, (inter_t) (lp->lp_elements[ec].element_multiplier));
					Produce::up();
					Produce::val(K_number, LITERAL_IVAL, 0);
				Produce::up();
				Produce::code();
				...
				Produce::down();
					...
					Produce::down();
						...
						Produce::down();
							...
							Produce::val(K_number, LITERAL_IVAL, (inter_t) (lp->lp_elements[ec].element_multiplier));
						Produce::up();
						Produce::val(K_number, LITERAL_IVAL, (inter_t) (lp->lp_elements[ec].element_range));
					Produce::up();
					Produce::val(K_number, LITERAL_IVAL, 0);
				Produce::up();
				Produce::code();
...
inter_symbol *LiteralPatterns::jump_label(literal_pattern *lp) {
	if (lp->jump_label == NULL) {
		...
		lp->jump_label = Produce::reserve_label(N);
		DISCARD_TEXT(N);
	}
