...
void LiteralPatterns::gpr(gpr_kit *gprk, literal_pattern *lp) {
	...
	Emit::down();
		Emit::ref_symbol(K_value, gprk->wpos_s);
		Emit::val(K_number, LITERAL_IVAL, 0);
	Emit::up();
	Emit::inv_primitive(Produce::opcode(STORE_BIP));
	Emit::down();
		Emit::ref_symbol(K_value, gprk->mid_word_s);
		Emit::val(K_truth_state, LITERAL_IVAL, 0);
	Emit::up();
	Emit::inv_primitive(Produce::opcode(STORE_BIP));
	Emit::down();
		Emit::ref_symbol(K_value, gprk->matched_number_s);
		Emit::val(K_number, LITERAL_IVAL, 0);
	Emit::up();

	TEMPORARY_TEXT(SL);
	WRITE_TO(SL, ".Succeeded_LP_%d", label);
	inter_symbol *succeeded_label = Emit::reserve_label(SL);
	DISCARD_TEXT(SL);
	TEMPORARY_TEXT(FL);
	WRITE_TO(FL, ".Failed_LP_%d", label);
	inter_symbol *failed_label = Emit::reserve_label(FL);
	DISCARD_TEXT(FL);
	...
	for (tc=0, ec=0; tc<lp->no_lp_tokens; tc++) {
		...
		Emit::down();
			...
			Emit::down();
				Emit::val_symbol(K_value, gprk->mid_word_s);
				Emit::val(K_truth_state, LITERAL_IVAL, 0);
			Emit::up();
			Emit::code();
			Emit::down();
	...
	Emit::down();
		...
		Emit::down();
			Emit::val_symbol(K_value, gprk->sgn_s);
			Emit::val(K_number, LITERAL_IVAL, 0);
		Emit::up();
		Emit::code();
		Emit::down();
			...
			Emit::down();
				...
				if (Kinds::FloatingPoint::uses_floating_point(lp->kind_specified)) {
					Emit::inv_primitive(Produce::opcode(BITWISEOR_BIP));
					Emit::down();
						Emit::val_symbol(K_value, gprk->matched_number_s);
						Emit::val(K_number, LITERAL_IVAL, 0x80000000);
					Emit::up();
				} else {
...
@<Compile I6 code to match a character token within a literal pattern@> =
	...
	Emit::inv_primitive(Produce::opcode(IF_BIP));
	Emit::down();
		if (upper_form != lower_form) { Emit::inv_primitive(Produce::opcode(AND_BIP)); Emit::down(); }
		Emit::inv_primitive(Produce::opcode(NE_BIP));
		Emit::down();
			Emit::inv_primitive(Produce::opcode(LOOKUPBYTE_BIP));
			Emit::down();
				Emit::val_symbol(K_value, gprk->cur_addr_s);
				Emit::inv_primitive(Produce::opcode(POSTINCREMENT_BIP));
				Emit::down();
					Emit::ref_symbol(K_value, gprk->wpos_s);
				Emit::up();
			Emit::up();
			Emit::val(K_number, LITERAL_IVAL, (inter_t) lower_form);
		Emit::up();
		if (upper_form != lower_form) {
			...
			Emit::down();
				...
				Emit::down();
					...
					Emit::down();
						Emit::val_symbol(K_value, gprk->wpos_s);
						Emit::val(K_number, LITERAL_IVAL, 1);
					Emit::up();
				Emit::up();
				Emit::val(K_number, LITERAL_IVAL, (inter_t) upper_form);
			Emit::up();
		}
...
@<Compile I6 code to match an element token within a literal pattern@> =
	...
	if (ec == 1) {
		Emit::inv_primitive(Produce::opcode(STORE_BIP));
		Emit::down();
			Emit::ref_symbol(K_value, gprk->sgn_s);
			Emit::val(K_number, LITERAL_IVAL, 1);
		Emit::up();
	}
	Emit::inv_primitive(Produce::opcode(IF_BIP));
	Emit::down();
		...
		Emit::down();
			...
			Emit::down();
				...
				Emit::val_symbol(K_value, gprk->wpos_s);
			Emit::up();
			Emit::val(K_number, LITERAL_IVAL, (inter_t) '-');
		Emit::up();
		Emit::code();
		Emit::down();
...
@<Compile I6 code to match a real number here@> =
	...
	Emit::down();
		...
		Emit::down();
			Emit::val_symbol(K_value, gprk->wpos_s);
			Emit::val_symbol(K_value, gprk->cur_len_s);
		Emit::up();
		Emit::code();
		Emit::down();
			Emit::inv_primitive(Produce::opcode(POSTINCREMENT_BIP));
	...
	Emit::inv_primitive(Produce::opcode(STORE_BIP));
	Emit::down();
		Emit::ref_symbol(K_value, gprk->tot_s);
		Emit::val(K_number, LITERAL_IVAL, 0);
	Emit::up();
		...
		Emit::down();
			...
			Emit::down();
				...
				Emit::down();
					...
					Emit::val_symbol(K_value, gprk->wpos_s);
				Emit::up();
				Emit::val(K_number, LITERAL_IVAL, (inter_t) 'x');
			Emit::up();
		Emit::up();
		Emit::code();
		Emit::down();
			...
			Emit::down();
				...
				Emit::down();
					...
					Emit::down();
						Emit::val_symbol(K_value, gprk->f_s);
						Emit::val_symbol(K_value, gprk->tot_s);
					Emit::up();
					Emit::val(K_number, LITERAL_IVAL, 1);
				Emit::up();
			Emit::up();
						...
						Emit::down();
							...
							Emit::down();
								...
								Emit::down();
									...
									Emit::up();
								Emit::up();
								Emit::val(K_number, LITERAL_IVAL, (inter_t) '0');
							Emit::up();
							Emit::inv_primitive(Produce::opcode(EQ_BIP));
							Emit::down();
								Emit::inv_primitive(Produce::opcode(LOOKUPBYTE_BIP));
	...
	Emit::down();
		...
		Emit::down();
			Emit::val_symbol(K_value, gprk->x_s);
			Emit::val_symbol(K_value, Hierarchy::veneer_symbol(FLOAT_NAN_VSYMB));
		Emit::up();
		Emit::code();
		Emit::down();
			Emit::inv_primitive(Produce::opcode(JUMP_BIP));
...
@<Compile I6 code to match an integer here@> =
	...
	if (lpe->element_index > 0) {
		...
		Emit::down();
			...
			Emit::down();
				Emit::val_symbol(K_value, gprk->tot_s);
				Emit::val(K_number, LITERAL_IVAL, (inter_t) lpe->element_range);
			Emit::up();
			Emit::code();
			Emit::down();
				Emit::inv_primitive(Produce::opcode(JUMP_BIP));
			...
			Emit::down();
				...
				Emit::down();
					...
					Emit::down();
						...
						Emit::down();
							Emit::ref_symbol(K_value, gprk->f_s);
							Emit::val(K_number, LITERAL_IVAL, (inter_t) M);
						Emit::up();
						Emit::inv_primitive(Produce::opcode(WHILE_BIP));
						Emit::down();
							Emit::inv_primitive(Produce::opcode(AND_BIP));
		...
		if (Wordings::nonempty(lpn->notation_name)) {
			...
			if (k > 0) {
				...
				Emit::down();
					Emit::inv_primitive(Produce::opcode(EQ_BIP));
					Emit::down();
						Emit::val_symbol(K_value, which_s);
						Emit::val(K_number, LITERAL_IVAL, (inter_t) (lpn->allocation_id + 1));
					Emit::up();
					Emit::code();
					Emit::down();
...
@<Compile code to jump to the correct printing pattern@> =
	...
	if ((lpb) && (lpb->singular_form_only == FALSE)) {
		...
		Emit::down();
			...
			Emit::down();
				Emit::inv_primitive(Produce::opcode(JUMP_BIP));
				Emit::down();
					Emit::lab(LiteralPatterns::jump_label(lpb));
				Emit::up();
			Emit::up();
		Emit::up();
	}
		...
		if (lp->marked_for_printing) {
			...
			Emit::down();
				...
				Emit::down();
					Emit::inv_primitive(Produce::opcode(JUMP_BIP));
					Emit::down();
						Emit::lab(LiteralPatterns::jump_label(lp));
					Emit::up();
				Emit::up();
			Emit::up();
		}
	...
	if (last_lp) {
		if ((last_lp->singular_form_only) &&
			...
			Emit::inv_primitive(Produce::opcode(JUMP_BIP));
			Emit::down();
				Emit::lab(LiteralPatterns::jump_label(last_plural));
			Emit::up();
		}
		Emit::inv_primitive(Produce::opcode(JUMP_BIP));
		Emit::down();
			Emit::lab(LiteralPatterns::jump_label(last_lp));
		Emit::up();
	}
				...
				Emit::down();
					...
					Emit::down();
						...
						Emit::down();
							...
							Emit::down();
								Emit::val_symbol(K_value, value_s);
								Emit::val(K_number, LITERAL_IVAL, (inter_t) (lpe->element_multiplier));
							Emit::up();
							Emit::val(K_number, LITERAL_IVAL, 0);
						Emit::up();
					Emit::up();
			...
			if (lp->number_signed) {
				...
				Emit::down();
					...
					Emit::down();
						Emit::inv_primitive(Produce::opcode(STORE_BIP));
						Emit::down();
							Emit::ref_symbol(K_value, value_s);
							Emit::inv_primitive(Produce::opcode(MINUS_BIP));
							Emit::down();
								Emit::val(K_number, LITERAL_IVAL, 0);
								Emit::val_symbol(K_value, value_s);
						...
						Emit::down();
							...
							Emit::down();
								...
								Emit::down();
									...
									Emit::val(K_number, LITERAL_IVAL, (inter_t) (lpe->element_range));
								Emit::up();
								Emit::val(K_number, LITERAL_IVAL, (inter_t) (pow));
							Emit::up();
							Emit::code();
							Emit::down();
								Emit::inv_primitive(Produce::opcode(PRINT_BIP));
		...
		if (ec == 0) {
			...
			Emit::down();
				...
				Emit::down();
					...
					Emit::down();
						Emit::val_symbol(K_value, value_s);
						Emit::val(K_number, LITERAL_IVAL, (inter_t) (lp->lp_elements[ec].element_multiplier));
					Emit::up();
					Emit::val(K_number, LITERAL_IVAL, 0);
				Emit::up();
				Emit::code();
				...
				Emit::down();
					...
					Emit::down();
						...
						Emit::down();
							...
							Emit::val(K_number, LITERAL_IVAL, (inter_t) (lp->lp_elements[ec].element_multiplier));
						Emit::up();
						Emit::val(K_number, LITERAL_IVAL, (inter_t) (lp->lp_elements[ec].element_range));
					Emit::up();
					Emit::val(K_number, LITERAL_IVAL, 0);
				Emit::up();
				Emit::code();
...
inter_symbol *LiteralPatterns::jump_label(literal_pattern *lp) {
	if (lp->jump_label == NULL) {
		...
		lp->jump_label = Emit::reserve_label(N);
		DISCARD_TEXT(N);
	}
