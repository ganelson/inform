...
@<Issue PM_RelationExists problem@> =
	*X = FALSE;
	Problems::Issue::sentence_problem(_p_(PM_RelationExists),
...
void Relations::parse_new_relation_further(parse_node *PN) {
	...
	if (Wordings::length(RW) > MAX_WORDS_IN_ASSEMBLAGE-4) {
		Problems::Issue::sentence_problem(_p_(PM_RelationNameTooLong),
...
@<Find term multiplicities and use of fast route-finding@> =
	...
	if (frf && (left_unique != FALSE) && (right_unique != FALSE)) {
		Problems::Issue::sentence_problem(_p_(PM_FRFUnavailable),
...
@<Detect use of a condition for a test-only relation@> =
	if (right_bitmap & WHEN_RBIT) {
		if ((left_unique != NOT_APPLICABLE) || (right_unique != NOT_APPLICABLE)) {
			Problems::Issue::sentence_problem(_p_(PM_OneOrVariousWithWhen),
...
@<Detect use of symmetry in definition of second term@> =
	...
	if ((specified_one == TRUE) && (left_unique == FALSE)) {
		Problems::Issue::sentence_problem(_p_(PM_BothOneAndMany),
...
@<Vet the use of callings for the terms of the relation@> =
	if (Wordings::empty(CONW)) {
		if ((left_unique == FALSE) && (Wordings::nonempty(LCALLW))) {
			Problems::Issue::sentence_problem(_p_(PM_CantCallLeft),
		...
		if ((right_unique == FALSE) && (Wordings::nonempty(RCALLW))) {
			Problems::Issue::sentence_problem(_p_(PM_CantCallRight),
				"the right-hand term of this relation is not unique",
				"so you cannot assign a name to it using 'called'.");
			return;
		}
		if ((Wordings::nonempty(LCALLW)) && (Wordings::nonempty(RCALLW))) {
			Problems::Issue::sentence_problem(_p_(PM_CantCallBoth),
		...
		if ((symmetric == FALSE) && (left_unique) && (right_unique) && (Wordings::nonempty(RCALLW))) {
			Problems::Issue::sentence_problem(_p_(PM_OneToOneMiscalled),
...
@<Issue a problem message since this won't be stored in a property@> =
	Problems::Issue::sentence_problem(_p_(PM_RelNotStoredInProperty),
...
int Relations::parse_relation_term_type(wording W, kind **set_K, char *side) {
	...
	*set_K = NULL;
	...
	Problems::Issue::handmade_problem(_p_(PM_RelatedKindsUnknown));
...
int Relations::check_finite_range(kind *K) {
	...
	if ((Kinds::Compare::le(K, K_object)) || (Kinds::Behaviour::definite(K) == FALSE))
		Problems::Issue::sentence_problem(_p_(PM_RangeOverlyBroad),
	...
	LOOP_OVER(bp, binary_predicate) {
		if ((bp->dynamic_memory) && (bp->right_way_round)) {
			...
			Produce::down();
				...
				Produce::val_iname(K_value, BinaryPredicates::iname(bp));
			Produce::up();

			Produce::inv_call_iname(Hierarchy::find(RELATION_TY_NAME_HL));
			Produce::down();
				Produce::val_iname(K_value, BinaryPredicates::iname(bp));
				TEMPORARY_TEXT(A);
			...
			switch(bp->form_of_relation) {
				case Relation_OtoO:
					Produce::inv_call_iname(Hierarchy::find(RELATION_TY_OTOOADJECTIVE_HL));
					Produce::down();
						Produce::val_iname(K_value, BinaryPredicates::iname(bp));
					...
					break;
				case Relation_OtoV:
					Produce::inv_call_iname(Hierarchy::find(RELATION_TY_OTOVADJECTIVE_HL));
					Produce::down();
						Produce::val_iname(K_value, BinaryPredicates::iname(bp));
						Produce::val(K_truth_state, LITERAL_IVAL, 1);
					Produce::up();
					break;
				case Relation_VtoO:
					Produce::inv_call_iname(Hierarchy::find(RELATION_TY_VTOOADJECTIVE_HL));
					Produce::down();
						Produce::val_iname(K_value, BinaryPredicates::iname(bp));
						Produce::val(K_truth_state, LITERAL_IVAL, 1);
					Produce::up();
				...
				case Relation_Sym_OtoO:
					...
					Produce::down();
						...
						Produce::val(K_truth_state, LITERAL_IVAL, 1);
					Produce::up();
					Produce::inv_call_iname(Hierarchy::find(RELATION_TY_SYMMETRICADJECTIVE_HL));
					Produce::down();
						Produce::val_iname(K_value, BinaryPredicates::iname(bp));
						Produce::val(K_truth_state, LITERAL_IVAL, 1);
					Produce::up();
					break;
				case Relation_Equiv:
					Produce::inv_call_iname(Hierarchy::find(RELATION_TY_EQUIVALENCEADJECTIVE_HL));
					Produce::down();
						Produce::val_iname(K_value, BinaryPredicates::iname(bp));
						Produce::val(K_truth_state, LITERAL_IVAL, 1);
					Produce::up();
			...
			Produce::inv_primitive(Produce::opcode(INDIRECT0V_BIP));
			Produce::down();
				Produce::val_iname(K_value, bp->initialiser_iname);
			Produce::up();
		}
	}
	Routines::end(save);
...
@<Write the relation handler routine for this BP@> =
	...
	Produce::down();
		Produce::val_symbol(K_value, task_s);
		Produce::code();
		Produce::down();
			Produce::inv_primitive(Produce::opcode(CASE_BIP));
			Produce::down();
				Produce::val_iname(K_value, Hierarchy::find(RELS_TEST_HL));
				Produce::code();
				Produce::down();
					@<The TEST task@>;
				Produce::up();
			...
			if (minimal) {
				...
				Produce::inv_primitive(Produce::opcode(CASE_BIP));
				Produce::down();
					Produce::val_iname(K_value, Hierarchy::find(RELS_LOOKUP_ANY_HL));
					Produce::code();
					Produce::down();
						@<The LOOKUP ANY task@>;
					Produce::up();
				...
				Produce::inv_primitive(Produce::opcode(CASE_BIP));
				Produce::down();
					Produce::val_iname(K_value, Hierarchy::find(RELS_LOOKUP_ALL_X_HL));
					Produce::code();
					Produce::down();
				...
				Produce::inv_primitive(Produce::opcode(CASE_BIP));
				Produce::down();
					Produce::val_iname(K_value, Hierarchy::find(RELS_LOOKUP_ALL_Y_HL));
					Produce::code();
					Produce::down();
				...
				Produce::inv_primitive(Produce::opcode(CASE_BIP));
				Produce::down();
					Produce::val_iname(K_value, Hierarchy::find(RELS_LIST_HL));
					Produce::code();
					Produce::down();
						@<The LIST task@>;
					Produce::up();
				...
				if (BinaryPredicates::can_be_made_true_at_runtime(bp)) {
					...
					Produce::inv_primitive(Produce::opcode(CASE_BIP));
					Produce::down();
						Produce::val_iname(K_value, Hierarchy::find(RELS_ASSERT_FALSE_HL));
						Produce::code();
						Produce::down();
				...
				if (shower) {
					Produce::inv_primitive(Produce::opcode(CASE_BIP));
					Produce::down();
						Produce::val_iname(K_value, Hierarchy::find(RELS_SHOW_HL));
						Produce::code();
						Produce::down();
							@<The SHOW task@>;
						Produce::up();
				...
				if (emptier) {
					Produce::inv_primitive(Produce::opcode(CASE_BIP));
					Produce::down();
						Produce::val_iname(K_value, Hierarchy::find(RELS_EMPTY_HL));
						Produce::code();
						Produce::down();
							@<The EMPTY task@>;
						Produce::up();
				...
				if (router) {
					Produce::inv_primitive(Produce::opcode(CASE_BIP));
					Produce::down();
						Produce::val_iname(K_value, Hierarchy::find(RELS_ROUTE_FIND_HL));
						Produce::code();
						Produce::down();
							@<The ROUTE FIND task@>;
						Produce::up();
					...
					Produce::inv_primitive(Produce::opcode(CASE_BIP));
					Produce::down();
						Produce::val_iname(K_value, Hierarchy::find(RELS_ROUTE_FIND_COUNT_HL));
						Produce::code();
						Produce::down();
...
@<The default case for minimal relations only@> =
	Produce::inv_call_iname(Hierarchy::find(RUNTIMEPROBLEM_HL));
	Produce::down();
		Produce::val_iname(K_value, Hierarchy::find(RTP_RELMINIMAL_HL));
		Produce::val_symbol(K_value, task_s);
...
@<The TEST task@> =
	...
	Produce::down();
		...
		for (int j=0; j<2; j++) {
			...
			if (fnsc) {
				if (j == 0) {
					...
					Produce::down();
						Produce::val_symbol(K_value, X_s);
					...
					Produce::inv_primitive(Produce::opcode(EQ_BIP));
					Produce::down();
						Produce::val_symbol(K_value, Y_s);
		...
		if (adapted == FALSE) {
			if (i6s == NULL) Produce::val(K_truth_state, LITERAL_IVAL, 0);
...
@<The ROUTE FIND task@> =
	...
	Produce::down();
		Produce::inv_primitive(Produce::opcode(INDIRECT3_BIP));
		Produce::down();
			Produce::val_iname(K_value, router);
...
@<Expand the ID operand@> =
	if (id_flag) {
		Produce::inv_call_iname(Hierarchy::find(RLNGETF_HL));
		Produce::down();
			Produce::val_symbol(K_value, rr_s);
			Produce::val_iname(K_value, Hierarchy::find(RR_STORAGE_HL));
		Produce::up();
	} else {
		Produce::val_symbol(K_value, rr_s);
	}
...
@<The ROUTE FIND COUNT task@> =
	...
	Produce::down();
	if (follow) {
		Produce::inv_call_iname(Hierarchy::find(RELFOLLOWVECTOR_HL));
		Produce::down();
			Produce::inv_primitive(Produce::opcode(INDIRECT3_BIP));
			Produce::down();
				Produce::val_iname(K_value, router);
		...
		Produce::inv_primitive(Produce::opcode(INDIRECT4_BIP));
		Produce::down();
			Produce::val_iname(K_value, router);
			@<Expand the ID operand@>;
			Produce::val_symbol(K_value, X_s);
...
@<The SHOW task@> =
	Produce::inv_primitive(Produce::opcode(INDIRECT2V_BIP));
	Produce::down();
		Produce::val_iname(K_value, shower);
		Produce::val_symbol(K_value, rr_s);
		Produce::val(K_truth_state, LITERAL_IVAL, (inter_t) par);
	Produce::up();
	Produce::rtrue();
...
@<The EMPTY task@> =
	...
	Produce::down();
		Produce::inv_primitive(Produce::opcode(INDIRECT3_BIP));
		Produce::down();
			Produce::val_iname(K_value, emptier);
			Produce::val_symbol(K_value, rr_s);
			Produce::val(K_truth_state, LITERAL_IVAL, (inter_t) par);
			Produce::inv_primitive(Produce::opcode(EQ_BIP));
			Produce::down();
...
@<The LOOKUP ANY task@> =
	...
	Produce::down();
		...
		Produce::down();
			...
			Produce::down();
				Produce::val_symbol(K_value, Y_s);
				Produce::val_iname(K_value, Hierarchy::find(RLANY_GET_X_HL));
			Produce::up();
			Produce::inv_primitive(Produce::opcode(EQ_BIP));
			Produce::down();
				Produce::val_symbol(K_value, Y_s);
				Produce::val_iname(K_value, Hierarchy::find(RLANY_CAN_GET_X_HL));
			Produce::up();
		Produce::up();
		Produce::code();
		Produce::down();
...
@<The LOOKUP ALL X task@> =
	Produce::inv_call_iname(Hierarchy::find(LIST_OF_TY_SETLENGTH_HL));
	Produce::down();
		Produce::val_symbol(K_value, Y_s);
		Produce::val(K_number, LITERAL_IVAL, 0);
	Produce::up();
...
@<The LIST task@> =
	...
	Produce::down();
		...
		Produce::down();
			Produce::val_symbol(K_value, Y_s);
			Produce::val_iname(K_value, Hierarchy::find(RLIST_ALL_X_HL));
		Produce::up();
		Produce::code();
		Produce::down();
			int t = 0;
		...
		Produce::down();
			...
			Produce::down();
				...
				Produce::down();
					Produce::val_symbol(K_value, Y_s);
					Produce::val_iname(K_value, Hierarchy::find(RLIST_ALL_Y_HL));
				Produce::up();
				Produce::code();
				Produce::down();
					t = 1;
...
@<Write rels lookup@> =
		...
		if (Calculus::Deferrals::write_loop_schema(&loop_schema, K)) {
			Calculus::Schemas::emit_expand_from_locals(&loop_schema, Z1_lv, Z2_lv, TRUE);
					...
					Produce::down();
						Produce::inv_primitive(Produce::opcode(INDIRECT4_BIP));
						Produce::down();
							Produce::val_iname(K_value, BinaryPredicates::handler_iname(dbp));
							Produce::val_symbol(K_value, rr_s);
							Produce::val_iname(K_value, Hierarchy::find(RELS_TEST_HL));
							if (t == 0) {
								Produce::val_symbol(K_value, Z1_s);
								Produce::val_symbol(K_value, X_s);
							} else {
								Produce::val_symbol(K_value, X_s);
								Produce::val_symbol(K_value, Z1_s);
						...
						Produce::down();
							...
							Produce::down();
								...
								Produce::down();
									Produce::val_symbol(K_value, Y_s);
									Produce::val_iname(K_value, Hierarchy::find(RLANY_CAN_GET_Y_HL));
								Produce::up();
								Produce::code();
								Produce::down();
									Produce::rtrue();
	...
	if (K == NULL) Produce::rfalse();
	else {
		Produce::inv_primitive(Produce::opcode(RETURN_BIP));
		Produce::down();
			Produce::inv_call_iname(Hierarchy::find(DEFAULTVALUEOFKOV_HL));
			Produce::down();
		...
		if (Calculus::Deferrals::write_loop_schema(&loop_schema, K)) {
			Calculus::Schemas::emit_expand_from_locals(&loop_schema, Z1_lv, Z2_lv, TRUE);
					...
					Produce::down();
						...
						Produce::code();
						Produce::down();
							Produce::inv_call_iname(Hierarchy::find(LIST_OF_TY_INSERTITEM_HL));
							Produce::down();
								Produce::val_symbol(K_value, Y_s);
								Produce::val_symbol(K_value, Z1_s);
							Produce::up();
					...
					Calculus::Schemas::emit_expand_from_locals(&loop_schema_R, Z3_lv, Z4_lv, TRUE);
							...
							Produce::down();
								...
								Produce::down();
									...
									Produce::val_iname(K_value, Hierarchy::find(RELS_TEST_HL));
									Produce::val_symbol(K_value, Z1_s);
									Produce::val_symbol(K_value, Z3_s);
								Produce::up();
								Produce::code();
								Produce::down();
									Produce::inv_call_iname(Hierarchy::find(LIST_OF_TY_INSERTITEM_HL));
...
void Relations::IterateRelations(void) {
	...
	LOOP_OVER(bp, binary_predicate)
		if (bp->record_needed) {
			...
			Produce::down();
				Produce::val_symbol(K_value, callback_s);
				Produce::val_iname(K_value, BinaryPredicates::iname(bp));
			Produce::up();
		}
	Routines::end(save);
...
@<Compile RProperty routine@> =
	...
	Produce::down();
		...
		Produce::down();
			Produce::val_symbol(K_value, obj_s);
			Produce::val_symbol(K_value, cl_s);
		Produce::up();
		Produce::code();
		Produce::down();
			Produce::inv_primitive(Produce::opcode(RETURN_BIP));
			Produce::down();
				...
				Produce::down();
					Produce::val_symbol(K_value, obj_s);
					Produce::val_symbol(K_value, pr_s);
				Produce::up();
			Produce::up();
		Produce::up();
	Produce::up();
		...
		if (rg->f0) {
			if (rg->check_R) {
				...
				Produce::down();
					...
					Produce::down();
						Produce::val_symbol(K_value, X_s);
						Produce::val_iname(K_value, Kinds::RunTime::I6_classname(rg->check_R));
					Produce::up();
					Produce::code();
					Produce::down();
			}
		...
		if (rg->f1) {
			if (rg->check_L) {
				...
				Produce::down();
					...
					Produce::down();
						Produce::val_symbol(K_value, X_s);
						Produce::val_iname(K_value, Kinds::RunTime::I6_classname(rg->check_L));
					Produce::up();
					Produce::code();
					Produce::down();
			}
			...
			Produce::down();
				...
				if (rg->check_L) {
					...
					Produce::down();
						Produce::inv_primitive(Produce::opcode(OFCLASS_BIP));
						Produce::down();
							Produce::val_symbol(K_value, L_s);
							Produce::val_iname(K_value, Kinds::RunTime::I6_classname(rg->check_L));
						Produce::up();
					downs++;
				}
				...
				for (int i=0; i<downs; i++) Produce::up();
	...
	if (rg->guard_make_true_iname) {
		...
		if (rg->inner_make_true) {
			...
			if (downs > 0) {
				...
				for (int i=0; i<downs-1; i++) Produce::up();
				Produce::code();
				Produce::down();
			}
			...
			if (downs > 0) { Produce::up(); Produce::up(); }

			Produce::inv_call_iname(Hierarchy::find(RUNTIMEPROBLEM_HL));
			Produce::down();
				Produce::val_iname(K_value, Hierarchy::find(RTP_RELKINDVIOLATION_HL));
				Produce::val_symbol(K_value, L_s);
				Produce::val_symbol(K_value, R_s);
				Produce::val_iname(K_value, rg->guarding->bp_iname);
			Produce::up();
		}
		Routines::end(save);
	}
...
void Relations::compile_routine_to_decide(inter_name *rname,
	...
	if ((spec == NULL) || (Dash::validate_conditional_clause(spec) == FALSE)) {
		Problems::Issue::sentence_problem(_p_(PM_BadRelationCondition),
