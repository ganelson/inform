...
@<Emit the op@> =
	if (strcmp(op, "==") == 0) Emit::inv_primitive(Produce::opcode(EQ_BIP));
	else if (strcmp(op, "~=") == 0) Emit::inv_primitive(Produce::opcode(NE_BIP));
	else if (strcmp(op, ">") == 0) Emit::inv_primitive(Produce::opcode(GT_BIP));
	else if (strcmp(op, ">=") == 0) Emit::inv_primitive(Produce::opcode(GE_BIP));
	else if (strcmp(op, "<") == 0) Emit::inv_primitive(Produce::opcode(LT_BIP));
	else if (strcmp(op, "<=") == 0) Emit::inv_primitive(Produce::opcode(LE_BIP));
				...
				Emit::down();
					...
					Emit::down();
						...
						Emit::down();
							Emit::inv_primitive(Produce::opcode(RETURN_BIP));
							Emit::down();
								Emit::inv_primitive(Produce::opcode(PLUS_BIP)); /* Plus one because we count the current turn */
								Emit::down();
									Emit::val_symbol(K_value, consecutives_s);
									Emit::val(K_number, LITERAL_IVAL, 1);
								Emit::up();
