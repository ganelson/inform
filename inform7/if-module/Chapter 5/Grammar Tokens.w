...
kind *PL::Parsing::Tokens::compile(gpr_kit *gprk, parse_node *pn, int code_mode,
	...
	if (bp) {
		Emit::inv_call(InterNames::to_symbol(InterNames::extern(ARTICLEDESCRIPTORS_EXNAMEF)));
		...
		if (bp == R_containment) {
			...
			Emit::down();
				...
				Emit::down();
					...
					Emit::down();
						Emit::val_iname(K_value, InterNames::extern(SELF_EXNAMEF));
						Emit::val_iname(K_value, InterNames::extern(CONTAINER_EXNAMEF));
					Emit::up();
				Emit::up();
				@<Then jump to our doom@>;
			Emit::up();
		...
		if (bp == R_support) {
			...
			Emit::down();
				...
				Emit::down();
					...
					Emit::down();
						Emit::val_iname(K_value, InterNames::extern(SELF_EXNAMEF));
						Emit::val_iname(K_value, InterNames::extern(SUPPORTER_EXNAMEF));
					Emit::up();
				Emit::up();
				@<Then jump to our doom@>;
			Emit::up();
		...
		if ((bp == a_has_b_predicate) || (bp == R_wearing) ||
			...
			Emit::down();
				...
				Emit::down();
					...
					Emit::down();
						Emit::val_iname(K_value, InterNames::extern(SELF_EXNAMEF));
						Emit::val_iname(K_value, InterNames::extern(ANIMATE_EXNAMEF));
					Emit::up();
				Emit::up();
				@<Then jump to our doom@>;
			Emit::up();
		...
		if ((bp == R_containment) ||
			...
			Emit::down();
				...
				Emit::down();
					...
					Emit::down();
						...
						Emit::down();
							Emit::val_symbol(K_value, gprk->w_s);
							Emit::inv_call(InterNames::to_symbol(InterNames::extern(TRYGIVENOBJECT_EXNAMEF)));
							Emit::down();
								Emit::val_symbol(K_value, gprk->rv_s);
			...
			Emit::down();
				...
				Emit::down();
					Emit::val_iname(K_value, InterNames::extern(SELF_EXNAMEF));
					Emit::val_iname(K_value, InterNames::extern(COMPONENTCHILD_EXNAMEF));
				Emit::up();
			Emit::up();
			...
			Emit::down();
				...
				Emit::down();
					...
					Emit::down();
						...
						Emit::down();
							Emit::val_symbol(K_value, gprk->rv_s);
							Emit::val_iname(K_value, InterNames::extern(COMPONENTSIBLING_EXNAMEF));
						Emit::up();
					Emit::up();
				Emit::up();
			Emit::up();
			...
			Emit::down();
				Emit::ref_symbol(K_value, gprk->rv_s);
				Emit::inv_call(InterNames::to_symbol(InterNames::extern(PARENT_EXNAMEF)));
				Emit::down();
					Emit::val_iname(K_value, InterNames::extern(SELF_EXNAMEF));
				Emit::up();
			Emit::up();
			...
			Emit::down();
				...
				Emit::down();
					Emit::val_iname(K_value, InterNames::extern(SELF_EXNAMEF));
					Emit::val_iname(K_value, InterNames::extern(COMPONENTPARENT_EXNAMEF));
				Emit::up();
			Emit::up();
			Emit::inv_primitive(store_interp);
			Emit::down();
			...
			if (i6s) {
				...
				if (Kinds::Compare::lt(K, K_object)) {
					...
					Emit::inv_primitive(objectloopx_interp);
					Emit::down();
						Emit::ref_symbol(K_value, gprk->rv_s);
				...
				if (prn) {
					if (reverse) {
						Emit::inv_primitive(if_interp);
						Emit::down();
							Emit::inv_primitive(provides_interp);
							Emit::down();
								Emit::val_iname(K_value, InterNames::extern(SELF_EXNAMEF));
		...
		if ((Kinds::Compare::le(K, K_object) == FALSE) &&
			...
			if (Kinds::Behaviour::offers_I6_GPR(K)) {
				...
				if (code_mode) {
					...
					Emit::down();
						Emit::ref_symbol(K_value, gprk->w_s);
						Emit::inv_call(InterNames::to_symbol(InterNames::extern(PARSETOKENSTOPPED_EXNAMEF)));
						Emit::down();
							...
							if (Str::len(i6_gpr_name) > 0)
								...
								Emit::val_iname(K_value, PL::Parsing::Tokens::Values::gprv_iname(K));
						Emit::up();
					Emit::up();
					Emit::inv_primitive(if_interp);
					Emit::down();
						...
						Emit::down();
							Emit::val_symbol(K_value, gprk->w_s);
							Emit::val_iname(K_number, InterNames::extern(GPRNUMBER_EXNAMEF));
						Emit::up();
						@<Then jump to our doom@>;
					Emit::up();
					Emit::inv_primitive(store_interp);
					...
					if (Str::len(i6_gpr_name) > 0)
						...
						Emit::array_iname_entry(PL::Parsing::Tokens::Values::gprv_iname(K));
				}
				return K;
			}
		...
		if (gtc < 0) {
			...
			if (code_mode) {
				...
				Emit::down();
					...
					Emit::inv_call(InterNames::to_symbol(InterNames::extern(PARSETOKENSTOPPED_EXNAMEF)));
					Emit::down();
						Emit::val_iname(K_value, InterNames::extern(ELEMENTARYTT_EXNAMEF));
					...
					if (Rvalues::is_CONSTANT_of_kind(spec, K_understanding)) {
						...
						if (code_mode) {
							...
							Emit::down();
								...
								Emit::down();
									Emit::val_symbol(K_value, gprk->w_s);
									Emit::val_iname(K_number, InterNames::extern(GPRPREPOSITION_EXNAMEF));
								Emit::up();
								Emit::code();
								Emit::down();
									Emit::inv_primitive(store_interp);
