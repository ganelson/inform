...
void PL::Parsing::Tokens::General::top_of_head(gpr_kit *gprk, inter_name *gv_iname, inference_subject *subj,
	...
	if ((gv_iname) && (sometimes_has_visible_properties == FALSE)) {
		...
		Emit::down();
			...
			Emit::down();
				Emit::val_iname(K_value, InterNames::extern(PARSERACTION_EXNAMEF));
				Emit::val_iname(K_value, InterNames::extern(THESAME_EXNAMEF));
			Emit::up();
			Emit::code();
			Emit::down();
				...
				Emit::down();
					...
					Emit::down();
						...
						Emit::down();
							Emit::inv_primitive(while_interp);
							Emit::down();
								Emit::inv_call(InterNames::to_symbol(InterNames::extern(WORDINPROPERTY_EXNAMEF)));
								Emit::down();
									...
									Emit::val_iname(K_value, InterNames::extern(NAME_EXNAMEF));
								Emit::up();
								Emit::code();
								Emit::down();
									Emit::inv_primitive(store_interp);
...
void PL::Parsing::Tokens::General::compile_parse_name_tail(gpr_kit *gprk) {
	...
	Emit::down();
		...
		Emit::up();
	Emit::up();
	Emit::inv_call(InterNames::to_symbol(InterNames::extern(DETECTPLURALWORD_EXNAMEF)));
	Emit::down();
		Emit::val_symbol(K_value, gprk->original_wn_s);
		Emit::val_symbol(K_value, gprk->n_s);
	Emit::up();
				...
				Emit::down();
					...
					Emit::down();
						...
						Emit::down();
							...
							Emit::inv_primitive(printnumber_interp);
							Emit::down();
								Emit::val_iname(K_value, InterNames::extern(PARSERONE_EXNAMEF));
							Emit::up();
							Emit::inv_primitive(print_interp);
							Emit::down();
								Emit::val_text(I", ");
							...
							Emit::inv_primitive(printnumber_interp);
							Emit::down();
								Emit::val_iname(K_value, InterNames::extern(PARSERTWO_EXNAMEF));
							Emit::up();
							Emit::inv_primitive(print_interp);
							Emit::down();
								Emit::val_text(I"\n");
...
void PL::Parsing::Tokens::General::test_distinguish_visible_property(gpr_kit *gprk, parse_node *spec) {
	Emit::inv_primitive(store_interp);
	Emit::down();
		Emit::ref_iname(K_value, InterNames::extern(SELF_EXNAMEF));
		Emit::val_iname(K_value, InterNames::extern(PARSERONE_EXNAMEF));
	Emit::up();
	Emit::inv_primitive(store_interp);
	Emit::down();
...
void PL::Parsing::Tokens::General::distinguish_visible_property(gpr_kit *gprk, property *prn) {
	...
	if (Properties::is_either_or(prn)) {
		...
		Emit::down();
			Emit::inv_primitive(and_interp);
			Emit::down();
				Properties::Emit::emit_has_property(K_value, InterNames::to_symbol(InterNames::extern(PARSERONE_EXNAMEF)), prn);
				Emit::inv_primitive(not_interp);
				Emit::down();
					Properties::Emit::emit_has_property(K_value, InterNames::to_symbol(InterNames::extern(PARSERTWO_EXNAMEF)), prn);
				Emit::up();
			Emit::up();
			Emit::code();
			Emit::down();
...
void PL::Parsing::Tokens::General::parse_visible_property(gpr_kit *gprk,
	...
	if (Properties::is_either_or(prn)) {
		...
		Emit::down();
			...
			Emit::val_iname(K_value, InterNames::extern(PARSEDNUMBER_EXNAMEF));
		Emit::up();
		Emit::inv_primitive(store_interp);
		Emit::down();
			Emit::ref_symbol(K_value, gprk->ss_s);
			Emit::val_iname(K_value, InterNames::extern(ETYPE_EXNAMEF));
		Emit::up();
		...
		Emit::inv_primitive(store_interp);
		Emit::down();
			Emit::ref_iname(K_value, InterNames::extern(ETYPE_EXNAMEF));
			Emit::val_symbol(K_value, gprk->ss_s);
		Emit::up();
	}
}
...
void PL::Parsing::Tokens::General::parse_visible_either_or(gpr_kit *gprk, property *prn, int visibility_level,
	...
	Emit::down();
		...
		Properties::Emit::emit_has_property(K_value, InterNames::to_symbol(InterNames::extern(SELF_EXNAMEF)), prn);
		int k = 0;
