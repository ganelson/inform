...
void PL::Parsing::Tokens::Filters::nft_compile_routine(noun_filter_token *nft) {
	...
	if (nft->parse_using_gpr) {
		...
		Produce::down();
			Produce::ref_symbol(K_value, v_s);
			...
			Produce::inv_call_iname(gpr_to_ask);
		Produce::up();
		...
		Produce::down();
			...
			Produce::down();
				Produce::inv_primitive(Produce::opcode(STORE_BIP));
				Produce::down();
					Produce::ref_symbol(K_value, n_s);
				...
				Produce::down();
					...
					Produce::down();
						...
						Produce::down();
							Produce::ref_symbol(K_value, v_s);
							Produce::val_iname(K_object, Hierarchy::find(GPR_FAIL_HL));
						Produce::up();
					Produce::up();
				Produce::up();
				...
				Produce::down();
					Produce::ref_iname(K_object, Hierarchy::find(NOUN_HL));
					Produce::val_symbol(K_value, n_s);
				Produce::up();
			Produce::up();
		Produce::up();
		...
		Produce::down();
			Produce::val_iname(K_object, Hierarchy::find(SCOPE_STAGE_HL));
			Produce::code();
			Produce::down();
				...
				Produce::down();
					...
					Produce::code();
					Produce::down();
						if (nft->any_things_flag) Produce::rtrue();
						else Produce::rfalse();
					Produce::up();
				Produce::up();
				Produce::inv_primitive(Produce::opcode(CASE_BIP));
				Produce::down();
					...
					Produce::down();
						Produce::inv_primitive(Produce::opcode(STORE_BIP));
						Produce::down();
							Produce::ref_symbol(K_value, obj_s);
							Produce::val_iname(K_object, Hierarchy::find(NOUN_HL));
						Produce::up();
						...
						Produce::down();
							...
							Produce::down();
								Produce::inv_primitive(Produce::opcode(STORE_BIP));
								Produce::down();
									Produce::ref_symbol(K_value, o2_s);
									Produce::val_iname(K_object, Hierarchy::find(NOUN_HL));
								Produce::up();
								Produce::inv_primitive(Produce::opcode(STORE_BIP));
								Produce::down();
								...
								Produce::inv_primitive(Produce::opcode(STORE_BIP));
								Produce::down();
									Produce::ref_iname(K_object, Hierarchy::find(SUPPRESS_SCOPE_LOOPS_HL));
									Produce::val(K_truth_state, LITERAL_IVAL, 1);
								Produce::up();

								Produce::inv_call_iname(Hierarchy::find(PLACEINSCOPE_HL));
								Produce::down();
									Produce::val_symbol(K_value, o2_s);
									Produce::val(K_truth_state, LITERAL_IVAL, 1);
								Produce::up();
				...
				Produce::down();
					...
					Produce::down();
						Produce::inv_primitive(Produce::opcode(STORE_BIP));
						Produce::down();
							Produce::ref_iname(K_object, Hierarchy::find(NEXTBEST_ETYPE_HL));
							Produce::val_iname(K_object, Hierarchy::find(NOTINCONTEXTPE_HL));
						Produce::up();
						Produce::inv_primitive(Produce::opcode(RETURN_BIP));
						Produce::down();
		...
		Produce::inv_primitive(Produce::opcode(STORE_BIP));
		Produce::down();
			Produce::ref_symbol(K_value, x_s);
			Produce::val_iname(K_object, Hierarchy::find(NOUN_HL));
		Produce::up();
...
void PL::Parsing::Tokens::Filters::compile_id(int id) {
	...
	LOOP_OVER(nft, noun_filter_token)
		if (nft->allocation_id == id) {
			if (nft->parse_using_gpr) Produce::val_iname(K_value, Hierarchy::find(GPR_TT_HL));
			else if (nft->global_scope_flag) Produce::val_iname(K_value, Hierarchy::find(SCOPE_TT_HL));
			else Produce::val_iname(K_value, Hierarchy::find(ROUTINEFILTER_TT_HL));
			Produce::val_iname(K_value, nft->nft_iname);
		}
}
