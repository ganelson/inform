...
void PL::Map::connect(inference_subject *i_from, inference_subject *i_to,
	...
	if (go_from == NULL) {
		...
		Problems::Issue::handmade_problem(_p_(PM_WayFromUnclear));
	...
	if ((reverse_dir) && (go_to) && (oneway_map_connections_only == FALSE)) {
		if (Instances::of_kind(reverse_dir, K_direction) == FALSE) {
			...
			Problems::Issue::handmade_problem(_p_(PM_OppositeNotDirection));
			Problems::issue_problem_segment(
...
void PL::Map::write_door_dir_routines(void) {
	...
	LOOP_OVER(notice, door_dir_notice) {
		...
		Produce::down();
			Produce::ref_symbol(K_value, loc_s);
			Produce::val_iname(K_value, Hierarchy::find(LOCATION_HL));
		Produce::up();
		...
		Produce::down();
			...
			Produce::down();
				Produce::val_symbol(K_value, loc_s);
				Produce::val_iname(K_value, Hierarchy::find(THEDARK_HL));
			Produce::up();
			Produce::code();
			Produce::down();
				Produce::inv_primitive(Produce::opcode(STORE_BIP));
				Produce::down();
					Produce::ref_symbol(K_value, loc_s);
					Produce::val_iname(K_value, Hierarchy::find(REAL_LOCATION_HL));
				Produce::up();
			Produce::up();
		Produce::up();
		...
		Produce::down();
			...
			Produce::down();
				Produce::val_symbol(K_value, loc_s);
				Produce::val_iname(K_value, Instances::iname(notice->R1));
			Produce::up();
			Produce::code();
			Produce::down();
				Produce::inv_primitive(Produce::opcode(RETURN_BIP));
				Produce::down();
					Produce::val_iname(K_value,
	...
	LOOP_OVER(notice, door_to_notice) {
		...
		Produce::down();
			...
			Produce::down();
				Produce::inv_primitive(Produce::opcode(RETURN_BIP));
				Produce::down();
					Produce::val_iname(K_value, Instances::iname(notice->R2));
				Produce::up();
			Produce::up();
		Produce::up();
